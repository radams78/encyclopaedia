...
\begin{prop}
The relation $\leq$ is a well ordering on $\omega$.
\end{prop}
...
\begin{proof}
	...
	\qedstep
\end{proof}
\step{4}{Any nonempty set $E$ of natural numbers has a least element.}
\begin{proof}
	\step{a}{\pflet{$E \subseteq \omega$}}
	\step{b}{\assume{$E$ has no least element.} \prove{$E = \emptyset$}}
	\step{c}{\pflet{$P(n)$ be the property $\forall m < n. m \notin E$}}
	\step{d}{$P(0)$}
	\begin{proof}
		\pf\ Vacuous.
	\end{proof}
	\step{e}{For any natural number $n$, if $P(n)$ then $P(n+1)$}
	\begin{proof}
		\pf\ If $P(n)$ then $n \notin E$ (otherwise $n$ would be the least element of $E$) and so $P(n+1)$.
	\end{proof}
	\step{f}{$\forall n \in \omega. P(n)$}
	\step{g}{$\forall n \in \omega. n \notin E$}
\end{proof}
\qed
\end{proof}
...
\begin{prop}
The lexicographical order is a partial order on $\omega \times \omega$.
\end{prop}
