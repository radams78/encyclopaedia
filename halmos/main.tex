...
\begin{df}[One-to-One]
A function $f : X \rightarrow Y$ is \emph{one-to-one} or 	\emph{injective} iff, for all $x,y \in X$, if $f(x) = f(y)$ then $x = y$.
\end{df}

\begin{df}[Onto]
Let $f : X \rightarrow Y$. We say $f$ is \emph{surjective}, or $f$ maps $X$ \emph{onto} $Y$ iff $\ran f = Y$.
\end{df}
...
\begin{proof}
\pf
\step{1}{\pflet{$y \in f(A)$}}
\step{2}{\pick\ $x \in A$ such that $f(x) = y$}
\step{3}{$x \in B$}
\step{4}{$y \in f(B)$}
\qed
\end{proof}
