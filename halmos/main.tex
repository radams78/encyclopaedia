...
\begin{df}
Let $f : X \rightarrow Y$ be one-to-one. Then we write $f^{-1}$ for the function $f^{-1} : \ran f \rightarrow X$ defined by: $f^{-1}(y)$ is the unique $x$ such that $f(x) = y$.
\end{df}

\begin{df}[Composite]
Let $f : X \rightarrow Y$ and $g : Y \rightarrow Z$. The \emph{composite} $g \circ f = g f : X \rightarrow Z$ is the function defined by
\[ (g \circ f)(x) = g(f(x)) \qquad (x \in X) \enspace . \]
\end{df}
...
Let $X = \{a,b\}$ where $a \neq b$. Define $f : X \rightarrow Y$ by $f(a) = f(b) = a$, and $g$ by $g(a) = g(b) = b$. Then $gf = g$ but $fg = f$.
\end{ex}
...
Let $R$ be a relation between $X$ and $Y$, and $S$ a relation between $Y$ and $Z$. The \emph{composite} $S \circ R = SR$ is the relation between $X$ and $Z$ defined by
