...
We write $x < y$ for $x \leq y \wedge x \neq y$. When this holds, we say $x$ is \emph{less than} $y$, \emph{smaller} than $y$, or a \emph{predecessor} of $y$.
\end{df}
...
\begin{proof}
	\step{a}{\assume{$m \leq n$ and $n \leq m$}}
	\step{b}{We cannot have $m < n$ and $n < m$}
	\begin{proof}
		\pf\ Then we would have $m < m$ by Proposition \ref{prop:natural_number_transitive}, contradicting Corollary \ref{cor:n_notin_n}.
	\end{proof}
	\step{c}{$m = n$}
\end{proof}
\step{3}{$\leq$ is transitive.}
\begin{proof}
	\pf\ Follows from Proposition \ref{prop:natural_number_transitive}.
\end{proof}
