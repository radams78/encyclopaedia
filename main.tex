...
\step{2}{\pflet{$m$ be the least element of $\mathbf{A} - \mathbf{B}$}}
\begin{proof}
...
Let $\mathbf{A}$ and $<$ be classes.
For any property $G(x,y)$, there exists a class $\mathbf{F}$ such that the following is a theorem:

Assume that $<$ is a well ordering on the class $\mathbf{A}$. Assume that, for any $f$, there exists a unique $y$ such that $G(f,y)$. Then $\mathbf{F} : \mathbf{A} \rightarrow \mathbf{V}$ is the unique function such that, for all $t \in \mathbf{A}$, we have $\mathbf{F} \restriction \seg t$ is a set and
 \[ G(\mathbf{F} \restriction \seg t, \mathbf{F}(t)) \enspace . \]
\end{thms}

\begin{proof}
\pf
\step{1}{For $t \in \mathbf{A}$, let us say that a function $v : \seg t \rightarrow \mathbf{V}$ is \emph{$G$-constructed up to $t$} iff 
\[ \forall x < t. G(v \restriction \seg x, v(x)) \enspace . \]}
\step{3}{Let $\mathbf{K}$ be the class of all functions $v$ such that there exists $t \in \mathbf{A}$ such that $v$ is $G$-constructed up to $t$.}
\step{4}{\pflet{$\mathbf{F} = \bigcup \mathbf{K}$}}
...
\begin{proof}
	\step{a}{\pflet{$(x,y_1),(x,y_2) \in \mathbf{F}$}}
	\step{b}{\pick\ $v_1,v_2 \in \mathbf{K}$ such that $v_1(x) = y_1$ and $v_2(x) = y_2$.}
	\step{c}{\pick\ $t_1, t_2 \in \mathbf{A}$ such that $v_1$ is $G$-constructed up to $t_1$ and $v_2$ is $G$-constructed up to $t_2$.}
	...
	\step{f}{$y_1 = y_2$}
\end{proof}
\step{6}{$\forall x \in \dom \mathbf{F}. G(\mathbf{F} \restriction \seg x, \mathbf{F}(x))$}
\begin{proof}
	\step{a}{\pflet{$x \in \dom F$}}
	\step{b}{\pick\ $v \in \mathbf{K}$ such that $v(x) = \mathbf{F}(x)$}
	\step{c}{\pick\ $t$ such that $v$ is $G$-constructed up to $t$.}
	\step{d}{$G(v \restriction \seg x, v(x))$}
	\step{e}{$v \restriction \seg x = \mathbf{F} \restriction \seg x$}
	\begin{proof}
		\step{i}{\pflet{$y < x$} \prove{$v(y) = \mathbf{F}(y)$}}
		\step{ii}{$y \in \dom \mathbf{F}$}
		\begin{proof}
			\pf\ Since $y \in \dom v$ and $v \in \mathbf{K}$.
		\end{proof}
		\step{iii}{\pick\ $u \in \mathbf{K}$ such that $u(y) = F(y)$}
		\step{iv}{$u(y) = v(y)$}
		\begin{proof}
			\pf\ \stepref{2}.
		\end{proof}
		\step{v}{$v(y) = \mathbf{F}(y)$}
	\end{proof}
	\step{f}{$G(\mathbf{F} \restriction \seg x, \mathbf{F}(x))$}
\end{proof}
\step{7}{$\dom \mathbf{F} = \mathbf{A}$}
\begin{proof}
	\step{a}{\assume{$\dom \mathbf{F} \neq \mathbf{A}$}}
	\step{b}{\pflet{$t$ be least in $\mathbf{A} - \dom \mathbf{F}$}}
	\step{c}{$\seg t = \dom \mathbf{F}$}
	\step{cc}{$\mathbf{F}$ is a set.}
	\begin{proof}
		\pf\ Axiom of Replacement.
	\end{proof}
	\step{d}{\pflet{$y$ be the unique object such that $G(\mathbf{F}, y)$}}
	\step{e}{\pflet{$v = \mathbf{F} \cup \{(t,y)\}$}}
	\step{f}{$v$ is $G$-constructed up to $t$.}
	\step{g}{$t \in \dom \mathbf{F}$}
	\qedstep
	\begin{proof}
		\pf\ This is a contradiction.
	\end{proof}
\end{proof}
\step{8}{$\mathbf{F}$ is unique.}
\begin{proof}
	\pf\ If $\mathbf{F}'$ also satisfies the theorem, prove $\mathbf{F}(x) = \mathbf{F}'(x)$ by transfinite induction on $x$.
\end{proof}
\qed
\end{proof}
...
\begin{proof}
	\pf\ Proposition \ref{prop:linord}.
\end{proof}
\step{4}{For any $\alpha \in \mathbf{On}$, the class $\{ \beta \in \mathbf{On} \mid \beta \mathbf{E} \alpha \}$ is a set.}
\begin{proof}
	\pf\ It is equal to $\alpha$.
\end{proof}
\step{5}{Every nonempty subset of $\mathbf{On}$ has an $\mathbf{E}$-least element.}
\begin{proof}
...
\begin{prop}
There is no function $f : \mathbb{N} \rightarrow \mathbf{V}$ such that $\forall n. f(n^+) \in f(n)$.
\end{prop}

\begin{proof}
\pf
\step{1}{\pflet{$f : \mathbb{N} \rightarrow \mathbf{V}$}}
\step{2}{\pick\ $m \in f(\mathbb{N})$ such that $m \cap f(\mathbb{N}) = \emptyset$}
\begin{proof}
	\pf\ Axiom of Regularity.
\end{proof}
\step{3}{\pick\ $n \in \mathbb{N}$ such that $m = f(n)$.}
\step{4}{$f(n) \cap f(\mathbb{N}) = \emptyset$}
\step{5}{$f(n^+) \notin f(n)$}
\qed
\end{proof}
...
\begin{prop}
\label{prop:pluslt}
For natural numbers $m$, $n$ and $p$, if $m < n$ then $m + p < n + p$.
\end{prop}

\begin{proof}
\pf
\step{1}{\pflet{$P[p]$ be the property: for any natural numbers $m$ and $n$, if $m < n$ then $m + p < n + p$.}}
\step{2}{$P[0]$}
\begin{proof}
	\pf\ Trivial.
\end{proof}
\step{3}{$\forall p \in \mathbb{N}. P[p] \Rightarrow P[p^+]$}
\begin{proof}
	\pf\ By Proposition \ref{prop:succltsucc}.
\end{proof}
\qed 
\end{proof}
...
\begin{proof}
	\step{a}{\pflet{$\lambda$ be a limit ordinal.} \prove{$\aleph_\lambda = \bigcup_{\alpha < \lambda} \aleph_\alpha$}}
	\step{b}{For all $\alpha < \lambda$ we have $\aleph_\alpha \leq \aleph_\lambda$}
	\begin{proof}
		\pf\ By definition of $\aleph_\lambda$.
	\end{proof}
	\step{c}{If $\forall \alpha < \lambda. \aleph_\alpha \leq \beta$ then $\aleph_\lambda \leq \beta$}
	\begin{proof}
		\pf\ By Lemma \ref{lm:supcard}.
	\end{proof}
\end{proof}
\step{2}{$\aleph$ is strictly monotone.}
\begin{proof}
