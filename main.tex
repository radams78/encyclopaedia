...
Let $\mathbf{R}$ be a linear ordering on $\mathbf{A}$ and $\mathbf{B} \subseteq \mathbf{A}$. Let $u \in \mathbf{A}$. Then $u$ is an \emph{upper bound} for $\mathbf{B}$ iff $\forall x \in \mathbf{B}. x \leq u$.
\end{df}

\begin{df}[Bounded]
Let $\mathbf{R}$ be a linear ordering on $\mathbf{A}$ and $\mathbf{B} \subseteq \mathbf{A}$. Then $\mathbf{B}$ is \emph{bounded above} iff it has an upper bound.
\end{df}

\begin{df}[Least Upper Bound]
Let $\mathbf{R}$ be a linear ordering on $\mathbf{A}$ and $\mathbf{B} \subseteq \mathbf{A}$. Let $s \in \mathbf{A}$. Then $s$ is the \emph{least upper bound} or \emph{supremum} of $\mathbf{B}$ iff $s$ is an upper bound for $\mathbf{B}$ and, for every upper bound $u$ for $\mathbf{B}$, we have $s \leq u$.
\end{df}
...
\begin{proof}
\pf
\step{1}{\pflet{$P(n)$ be the property: For any natural number $m$ and sets $A$ and $B$, if $A$ has $m$ elements, $B$ has $n$ elements, and $A \cap B = \emptyset$, then $A \cup B$ has $m + n$ elements.}}
\step{1}{$P(0)$}
\begin{proof}
	\step{a}{\pflet{$A$ have $m$ elements, $B$ have 0 elements, and $A \cap B = \emptyset$}}
	\step{b}{$B = \emptyset$}
	\step{c}{$A \cup B = A$}
	\step{d}{$A \cup B$ has $m$ elements.}
\end{proof}
\step{2}{$\forall n. P(n) \rightarrow P(n+1)$}
\begin{proof}
	\step{a}{\assume{$P(n)$}}
	\step{a}{\pflet{$A$ have $m$ elements, $B$ have $n+1$ elements, and $A \cap B = \emptyset$}}
	\step{b}{\pick\ $b \in B$}
	\step{c}{$B - \{b\}$ has $n$ elements.}
	\step{d}{$A \cup B - \{b\}$ has $m + n$ elements.}
	\step{e}{$A \cup B$ has $m + n + 1$ elements.}
\end{proof}
\step{3}{$\forall n P(n)$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$P(n)$ be the property: for any natural number $m$ and sets $A$ and $B$, if $A$ has $m$ elements and $B$ has $n$ elements then $A \times B$ has $mn$ elements.}}
\step{2}{$P(0)$}
\begin{proof}
	\step{a}{\pflet{$A$ have $m$ elements and $B$ have 0 elements.}}
	\step{b}{$B = \emptyset$}
	\step{c}{$A \times B = \emptyset$}
	\step{d}{$A \times B$ has 0 elements.}
\end{proof}
\step{3}{$\forall n. P(n) \rightarrow P(n+1)$}
\begin{proof}
	\step{a}{\assume{$P(n)$}}
	\step{b}{\pflet{$A$ have $m$ elements and $B$ have $n+1$ elements.}}
	\step{c}{\pick\ $b \in B$}
	\step{d}{$B - \{b\}$ has $n$ elements.}
	\step{e}{$A \times (B - \{b\})$ has $mn$ elements.}
	\step{f}{$A \times B = (A \times (B - \{b\})) \cup (A \times \{b\})$}
	\step{g}{$A \times \{b\}$ has $m$ elements.}
	\begin{proof}
		\pf\ It is bijective with $A$.
	\end{proof}
	\step{h}{$A \times B$ has $mn+m$ elements.}
	\begin{proof}
		\pf\ Proposition \ref{prop:cardsum}.
	\end{proof}
\end{proof}
\step{4}{$\forall n P(n)$}
\qed
...
\begin{prop}
The inverse of an element in an Abelian group is unique.
\end{prop}
...
We may also say we are writing an Abelian group \emph{additively}, meaning we write $a + b$ for $ab$, $0$ for $e$ and $-a$ for $a^{-1}$. In this case we write $a-b$ for $ab^{-1}$.
\end{df}

\begin{prop}
\label{prop:Abgroupcancel}
In any Abelian group, if $ab = ac$ then $b = c$.
\end{prop}
...
An \emph{integral domain} consists of a set $D$ and two binary operations $+$, $\cdot$ on $D$ such that:
\begin{itemize}
\item
...
\item
$\cdot$ has an identity element that is different from 0.
\item
For all $a,b \in D$, if $ab = 0$ then $a = 0$ or $b = 0$.
\end{itemize}
\end{df}

\begin{prop}
\label{prop:timeszero}
In any integral domain, $0x = 0$.
\end{prop}
...
(0+0)x & = 0x \\
\therefore 0x + 0x & = 0x + 0 \\
\therefore 0x & = 0 & (\text{Proposition \ref{prop:Abgroupcancel}}) \qed
\end{align*}
\end{proof}

\begin{prop}
In any integral domain, $(-a)b = -(ab)$.
\end{prop}
...
\qed
\end{proof}
\step{12}{For any $x,y \in F$, if $xy = [(0,1)]$ then $x = [(0,1)]$ or $y = [(0,1)]$.}
\begin{proof}
\pf\ If $qr = 0$ and $q \neq 0$, pick $q' \in F$ such that $qq' = 1$. Then $r = 1r = q'qr = q' 0 = 0$.
\end{proof}
\qed
\end{proof}
...
An \emph{ordered field} consists of a field $F$ with a linear order $<$ on $F$ such that:
\begin{itemize}
\item for all $x,y,z \in F$, we have $x < y$ if and only if $x + z < y + z$.
\item for all $x,y,z \in F$, if $0 < z$ then we have $x < y$ if and only if $xz < yz$.
\end{itemize}
\end{df}
...
\begin{prop}
The ordering on an ordered field is dense; that is, if $x < y$ then there exists $z$ such that $x < z < y$.
\end{prop}
...
\begin{proof}
	\pf\ Proposition \ref{prop:Abgroupcancel}.
\end{proof}
