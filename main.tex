...
%TODO Define topological space
%TODO Define continuous function
\begin{df}
...
%TODO Define convergent sequence
\begin{prop}
...
\item $\mathcal{C}(\Omega)$ for the normed space $\mathcal{C}(\Omega)$ under the norm $\| f \| = \max_{x \in \Omega} |f(x)|$
\item $l^p$ for the normed space $l^p$ under the norm $\| (z_n) \| = \left( \sum_{n=0}^\infty |z_n|^p \right)^{1/p}$.
\end{itemize}

\begin{df}[Convergence]
Let $K$ be either $\mathbb{R}$ or $\mathbb{C}$. Let $V$ be a normed space over $K$. Let $(x_n)$ be a sequence in $V$ and $l \in V$. Then $x_n$ \emph{converges} to $l$, $x_n \rightarrow l$ as $n \rightarrow \infty$, iff for all $\epsilon > 0$ there exists $N$ such that, for all $n \geq N$, we have $\| x_n - l \| < \epsilon$.
\end{df}

\begin{prop}
Let $K$ be either $\mathbb{R}$ or $\mathbb{C}$. Let $V$ be a normed space over $K$. Let $(x_n)$ be a sequence in $V$ and $l \in V$. Then $x_n \rightarrow l$ as $n \rightarrow \infty$ in $V$ if and only if $\| x_n - l \| \rightarrow 0$ as $n \rightarrow \infty$ in $\mathbb{R}$.
\end{prop}
...
\begin{prop}
A convergent sequence in a normed space has at most one limit.
\end{prop}

\begin{proof}
\pf
\step{1}{\assume{$x_n \rightarrow l$ and $x_n \rightarrow m$ as $n \rightarrow \infty$}}
\step{2}{$\| l - m \| = 0$}
\begin{proof}
	\step{a}{\pflet{$\epsilon > 0$} \prove{$\| l - m \| < \epsilon$}}
	\step{b}{\pick\ $N$ such that $\forall n \geq N. \| x_n - l \| < \epsilon / 2$ and $\forall n \geq N. \| x_n - m \| < \epsilon / 2$}
	\step{c}{$\| l - m \| < \epsilon$}
	\begin{proof}
		\pf\ $\| l - m \| \leq \| x_N - l \| + \| x_N - m \| < \epsilon$
	\end{proof}
\end{proof}
\step{3}{$l = m$}
\qed
