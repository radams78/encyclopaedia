...
For $\bigcap \mathbf{V}$, see Proposition \ref{prop:bigcapV}.
...
\begin{prop}
\label{prop:bigcapV}
\[ \bigcap \mathbf{V} = \emptyset \]
\end{prop}

\begin{proof}
\pf\ If $x \in \bigcap \mathbf{V}$ then $x \in \emptyset$ which is impossible. \qed
\end{proof}
...
\begin{proof}
\step{1}{\pflet{$A = \{ \{ \emptyset \} \}$ and $B = \{ \{ \emptyset, \{ \emptyset \} \} \}$}}
\step{2}{$\bigcup(A \cap B) = \emptyset$}
\step{3}{$\bigcup A \cap \bigcup B = \{ \emptyset \}$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{$\bigcup \mathbf{V} \subseteq \mathbf{V}$}
\begin{proof}
	\pf\ Trivial.
\end{proof}
\step{2}{$\mathbf{V} \subseteq \bigcup \mathbf{V}$}
\begin{proof}
