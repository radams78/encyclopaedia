...
If $\alpha$ and $\beta$ are ordinals then $\alpha \cap \beta$ is an ordinal.
\end{prop}

\begin{proof}
\pf
\step{1}{$\alpha \cap \beta$ is a transitive set.}
\begin{proof}
	\pf\ Proposition \ref{prop:inttransitive}.
\end{proof}
\step{2}{$\forall x,y \in \alpha \cap \beta. x \in y \vee x = y \vee y \in x$}
\begin{proof}
	\pf\ From the same condition in $\alpha$.
\end{proof}
\qed
\end{proof}
