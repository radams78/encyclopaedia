...
\begin{df}[Ordinal]
An \emph{ordinal (number)} is a transitive set $\alpha$ such that, for all $x, y \in \alpha$, we have $x \in y$ or $x = y$ or $y \in x$.
...
\begin{proof}
	...
	\step{f}{$\alpha \notin x$}
	\begin{proof}
		\pf\ We cannot have $x \in y \in \alpha \in x$ by the Axiom of Foundation.
	\end{proof}
	\step{g}{$x \in \alpha$}
\end{proof}
\step{4}{$\forall x,y \in \alpha. x \in y \vee x = y \vee y \in x$}
\begin{proof}
	\pf\ This holds because if $x,y \in \alpha$ then $x,y \in \beta$ since $\beta$ is a transitive set.
\end{proof}
\qed
\end{proof}
