...
\step{2}{\pflet{$i,j : A \rightarrow A$ be identity functions.}}
\step{3}{$i = j$}
\begin{proof}
...
\step{2}{\pflet{$j$ be the unique function $T' \rightarrow T$.}}
...
\begin{proof}
\pf
\step{1}{\pflet{$\phi : P \rightarrow Q$ be the unique function such that $q_1 \circ \phi = p_1$ and $q_2 \circ \phi = p_2$.}}
\step{2}{\pflet{$\phi^{-1} : Q \rightarrow P$ be the unique function such that $p_1 \circ \phi = q_1$ and $p_2 \circ \phi = q_2$.}}
...
\begin{proof}
\pf
\step{1}{\pflet{$r : A \rightarrow B$ be a monic retraction of $s : B \rightarrow A$.}}
\step{2}{$rsr = r$}
\step{3}{$sr = \id{A}$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$\phi = r' \circ s$}}
\step{2}{\pflet{$\phi^{-1} = r \circ s'$}}
...
\begin{proof}
	...
	\end{align*}
\end{proof}
\step{7}{If $\psi : B \cong B'$ satisfies $\psi r = r'$ and $s' \psi = s$ then $\psi = \phi$.}
\begin{proof}
	\pf
	\begin{align*}
		\psi & = \psi r s \\
...
Hence we may talk about \emph{the} splitting of a split idempotent.

\begin{prop}
If $e : A \rightarrow A$ is idempotent and either monic or epi, then $e = \id{A}$.
\end{prop}

\begin{proof}
\pf\ We have $ee = e$ and so $e = \id{A}$ by cancellation. \qed
\end{proof}

\section{Involutions}

\begin{df}[Involution]
An \emph{involution} on an object $A$ is an automorphism $f : A \cong A$ such that $ff = f$.
\end{df}

\chapter{Terminal Objects}

\begin{df}[Terminal Object]
An object $T$ is \emph{terminal} iff, for any object $X$, there is exactly one morphism $X \rightarrow T$.
\end{df}
...
\begin{proof}
\pf
\step{1}{\pflet{$\phi$ be the unique morphism $T \rightarrow T'$.}}
\step{2}{\pflet{$\inv{\phi}$ be the unique morphism $T' \rightarrow T$.}}
\step{3}{$\phi \circ \inv{\phi} = \id{T'}$}
\begin{proof}
	\pf\ Since there is only one morphism $T' \rightarrow T'$.
\end{proof}
\step{4}{$\inv{\phi} \circ \phi = \id{T}$}
\begin{proof}
	\pf\ Since there is only one morphism $T \rightarrow T$.
\end{proof}
\qed
...
\chapter{Initial Objects}

\begin{df}[Initial Object]
An object $I$ is \emph{initial} iff, for any object $X$, there is exactly one morphism $I \rightarrow X$.
\end{df}
...
\begin{proof}
\pf
\step{1}{\pflet{$\phi$ be the unique morphism $I \rightarrow I'$.}}
\step{2}{\pflet{$\inv{\phi}$ be the unique morphism $I' \rightarrow I$.}}
\step{3}{$\phi \circ \inv{\phi} = \id{I'}$}
\begin{proof}
	\pf\ Since there is only one morphism $I' \rightarrow I'$.
\end{proof}
\step{4}{$\inv{\phi} \circ \phi = \id{I}$}
\begin{proof}
	\pf\ Since there is only one morphism $I \rightarrow I$.
\end{proof}
\qed
...
\begin{proof}
\pf
\step{1}{\pflet{$\alpha = 1 / \| A^{-1} \|$}}
\step{2}{\pflet{$\beta = \| B - A \|$}}
\step{3}{$\beta < \alpha$}
\step{4}{For all $\vec{x} \in \mathbb{R}^n$ we have $\alpha \| \vec{x} \| \leq \beta \| \vec{x} \| + \| B \vec{x} \|$}
\begin{proof}
	\pf\ Lemma \ref{lm:pre_norm_inverse}.
\end{proof}
\step{5}{For all $\vec{x} \in \mathbb{R}^n$ we have
\[ (\alpha - \beta) \| \vec{x} \| \leq \| B \vec{x} \| \enspace . \]}
\step{6}{$\ker B = 0$}
\begin{proof}
...
\begin{prop}
The set $\Omega$ of all invertible linear transformations is an open set in $\mathbf{Vect}_\mathbb{R}[\mathbb{R}^n, \mathbb{R}^n]$.
\end{prop}

\begin{proof}
\pf\ For $A \in \Omega$ we have $B(A, 1/\|A^{-1}\|) \subseteq \Omega$ by Proposition \ref{prop:norm_inverse}. \qed
\end{proof}
...
\step{2}{\pick\ $N$ such that, for all $n \geq N$, we have $\|s_n - s \| < \epsilon / 2$ and $\| t_n - t \| < \epsilon / 2$.}
\step{3}{For all $n \geq N$ we have $\| (s_n + t_n) - (s + t) \| < \epsilon$.}
\begin{proof}
...
\begin{prop}
\label{prop:nested_closed_bounded_sets}
Let $X$ be a complete metric space. Let $(E_n)$ be a sequence of nonempty closed bounded sets in $X$ with
\[ E_0 \supseteq E_1 \supseteq \cdots \]
and $\diam E_n \rightarrow 0$ as $n \rightarrow \infty$. Then $\bigcap_{n=0}^\infty E_n$ consists of exactly one point.
\end{prop}

\begin{proof}
\pf
\step{1}{\pflet{$K = \bigcap_{n=0}^\infty E_n$}}
\step{2}{$K$ has at least one point.}
\begin{proof}
	\step{a}{For each $n$, \pick\ $a_n \in E_n$}
	\step{b}{$(a_n)$ is Cauchy.}
	\begin{proof}
		\step{i}{\pflet{$\epsilon > 0$}}
		\step{ii}{\pick\ $N$ such that $\forall n \geq N. \diam E_n < \epsilon$}
		\step{iii}{$\forall m,n \geq N. d(a_m,a_n) < \epsilon$}
	\end{proof}
	\step{c}{\pflet{$l = \lim_{n \rightarrow \infty} a_n$}}
	\step{d}{$l \in K$}
	\begin{proof}
		\step{i}{\pflet{$n \in \mathbb{N}$}}
		\step{ii}{For all $m \geq n$ we have $a_m \in E_n$}
		\step{iii}{$l \in E_n$}
	\end{proof}
\end{proof}
\step{3}{$K$ has at most one point.}
\begin{proof}
	\step{a}{\assume{for a contradiction $a,b \in K$ such that $a \neq b$}}
	\step{b}{\pick\ $n$ such that $\diam E_n < d(a,b)$}
	\step{c}{$a,b \in E_n$}
	\qedstep
	\begin{proof}
		\pf\ This is a contradiction.
...
Let $X$ be a complete metric space. Let $(G_n)$ be a sequence of dense open subsets of $X$. Then $\bigcap_{n=0}^\infty G_n$ is not empty.
\end{thm}

\begin{proof}
\pf
\step{1}{\pick\ a sequence $(E_n)$ of open balls such that $E_0 \supseteq E_1 \supseteq \cdots$ and $\diam E_n \leq 1/2^n$ and $\overline{E_n} \subseteq G_n$.}
\begin{proof}
	\step{a}{\assume{as induction hypothesis we have chosen $E_0$, \ldots, $E_n$ with centres $c_0$, \ldots, $c_n$.}}
	\step{b}{\pick\ $x \in E_n \cap G_{n+1}$}
	\step{c}{\pick\ $0 < \epsilon \leq 1/2^{n+2}$ such that $B(x, \epsilon) \subseteq E_n \cap G_{n+1}$}
	\step{d}{\pflet{$E_{n+1} = B(x, \epsilon/2)$}}
	\step{e}{$E_{n+1} \subseteq E_n$}
	\step{f}{$\diam E_{n+1} \leq 1/2^{n+1}$}
	\step{g}{$\overline{E_{n+1}} \subseteq G_{n+1}$}
\end{proof}
\step{2}{\pflet{$\bigcap_{n=0}^\infty \overline{E_n} = \{ p \}$}}
\begin{proof}
	\pf\ Proposition \ref{prop:nested_closed_bounded_sets}.
\end{proof}
\step{3}{$p \in \bigcap_{n=0}^\infty G_n$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$\alpha = \limsup_n c_{n+1}/c_n$}}
\step{2}{\assume{w.l.o.g. $\alpha < + \infty$}}
\step{3}{For all $\beta > \alpha$ we have $\limsup_{n \rightarrow \infty} c_n^{1/n} \leq \beta$.}
\begin{proof}
	\step{a}{\pflet{$\beta > \alpha$}}
	\step{b}{\pick\ $N$ such that, for all $n \geq N$, we have
	\[ \frac{c_{n+1}}{c_n} \leq \beta \enspace . \]}
	\begin{proof}
		\pf\ Theorem \ref{thm:greater_than_limsup}.
	\end{proof}
	\step{c}{For all $k \geq 0$ we have
	\[ c_{N + k + 1} \leq \beta c_{N + k} \enspace . \]}
	\step{d}{For all $n \geq N$ we have
	\[ c_n \leq c_N \beta^{-N} \beta^n \enspace . \]}
	\begin{proof}
		\pf\ Induction on $n$.
	\end{proof}
	\step{e}{For all $n \geq N$ we have
	\[ c_n^{1/n} \leq (c_N \beta^{-N})^{1/n} \beta \enspace . \]}
	\step{f}{\[ \limsup_{n \rightarrow \infty} c_n^{1/n} \leq \beta \]}
	\begin{proof}
		\pf
		\begin{align*}
	...
	\end{proof}
\end{proof}
\step{4}{\[ \limsup_{n \rightarrow \infty} c_n^{1/n} \leq \alpha \]}
\qed
\end{proof}
...
\step{3}{$\forall m,n \geq N. \left\|\sum_{k=m}^n a_k\right\| \leq \epsilon$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{If $\alpha < 1$ then $\sum_{n=1}^\infty a_n$ converges.}
\begin{proof}
	\step{a}{\assume{$\alpha < 1$}}
	\step{b}{\pick\ $\beta$ such that $\alpha < \beta < 1$}
	\step{c}{\pick\ $N$ such that $\forall n \geq N. \|a_n\|^{1/n} < \beta$}
	\begin{proof}
		\pf\ Theorem \ref{thm:greater_than_limsup}.
	\end{proof}
	\step{d}{$\forall n \geq N. \|a_n\| < \beta^n$}
	\step{e}{$\sum_{n=1}^\infty \beta^n$ converges.}
	\begin{proof}
		\pf\ Theorem \ref{thm:geometric_series}.
	\end{proof}
	\step{f}{$\sum_{n=1}^\infty a_n$ converges.}
	\begin{proof}
		\pf\ Comparison Test.
	\end{proof}
\end{proof}
\step{2}{If $\alpha > 1$ then $\sum_{n=1}^\infty a_n$ diverges.}
\begin{proof}
	\step{a}{\assume{$\alpha > 1$}}
	\step{b}{There exists a sequence of positive integers $(n_k)$ such that $\|a_{n_k}\|^{1/n_k} \rightarrow \alpha$ as $k \rightarrow \infty$.}
	\begin{proof}
		\pf\ Theorem \ref{thm:limsup_is_subsequential_limit}.
	\end{proof}
	\step{c}{There are infinitely many $n$ such that $\|a_n\| > 1$.}
	\step{d}{$a_n \nrightarrow 0$ as $n \rightarrow \infty$.}
	\step{e}{$\sum_{n=1}^\infty a_n$ diverges.}
	\begin{proof}
...
\begin{proof}
\pf
\step{1}{If $\limsup_{n \rightarrow \infty} \frac{\|a_{n+1}\|}{\|a_n\|} < 1$
then $\sum_{n=0}^\infty a_n$ converges.}
\begin{proof}
	\step{a}{\assume{$\limsup_{n \rightarrow \infty} \frac{\|a_{n+1}\|}{\|a_n\|} < 1$}}
	\step{b}{$\limsup_{n \rightarrow \infty} \|a_n\|^{1/n} < 1$}
	\begin{proof}
		\pf\ Theorem \ref{thm:limsups_compared}.
	\end{proof}
	\step{c}{$\sum_{n=0}^\infty a_n$ converges.}
	\begin{proof}
		\pf\ Root Test
	\end{proof}
\end{proof}
\step{2}{If there exists $N$ such that $\forall n \geq N. \frac{\|a_{n+1}\|}{\|a_n\|} > 1$ then
$\sum_{n=0}^\infty a_n$ diverges.}
\begin{proof}
	\pf\ Since $a_n \nrightarrow 0$ as $n \rightarrow \infty$.
\end{proof}
...
\begin{proof}
\pf
\step{1}{For $n \in \mathbb{N}$, \pflet{$s_n = \sum_{k=0}^n \frac{1}{k!}$}}
\step{2}{For $n \in \mathbb{Z}^+$, \pflet{$t_n = \left( 1 + \frac{1}{n} \right)^n$}}
\step{3}{For $n \in \mathbb{Z}^+$ we have
\[ t_n = \sum_{k=0}^n \frac{1}{k!} \prod_{i=1}^{k-1} \left( 1 - \frac{i}{n} \right) \enspace . \]}
\begin{proof}
\pf
\begin{align*}
...
\end{align*}
\end{proof}
\step{4}{For $n \in \mathbb{Z}^+$ we have $t_n \leq s_n$.}
\step{5}{$\limsup_{n \rightarrow \infty} t_n \leq e$}
\step{6}{For $m,n \in \mathbb{Z}^+$ with $n \geq m$ we have
\[ t_n \geq \sum_{k=0}^m \frac{1}{k!} \prod_{i=1}^{k-1} \left( 1 - \frac{i}{n} \right) \enspace . \]}
\step{7}{For $m \in \mathbb{Z}^+$ we have
\[ \liminf_{n \rightarrow \infty} t_n \geq \sum_{k=0}^m \frac{1}{k!} \enspace . \]}
\step{8}{For $m \in \mathbb{Z}^+$ we have
\[ s_m \leq \liminf_{n \rightarrow \infty} t_n \enspace . \]}
\step{9}{\[ e \leq \liminf_{n \rightarrow \infty} t_n \]}
\step{10}{$t_n \rightarrow e$ as $n \rightarrow \infty$.}
\begin{proof}
	\pf\ From \stepref{5} and \stepref{9}.
\end{proof}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\assume{for a contradiction $e = p/q$ where $p$ and $q$ are positive integers.}}
\step{2}{For $n \in \mathbb{N}$, \pflet{$s_n = \sum_{k=0}^n \frac{1}{k!}$.}}
\step{3}{For $n \in \mathbb{Z}^+$ we have
\[ 0 < e - s_n < \frac{1}{n!n} \enspace . \]}
\begin{proof}
	\pf
	\begin{align*}
	...
	\end{align*}
\end{proof}
\step{4}{\[ 0 < q!(e - s_q) < \frac{1}{q} \]}
\step{5}{$q!e$ is an integer.}
\step{6}{$q!(e - s_q)$ is an integer.}
\step{7}{There exists an integer between 0 and 1.}
\qedstep
\begin{proof}
	\pf\ This is a contradiction.
\end{proof}
...
\begin{proof}
\pf
\step{1}{For $z \in \mathbb{C}$ and $n \in \mathbb{N}$, \pflet{$a_n(z) = c_n z^n$}}
\step{2}{\[ \limsup_{n \rightarrow \infty} |a_n(z)|^{1/n} = |z| / R \]}
\step{3}{If $|z| < R$ then $\sum_{n=0}^\infty a_n(z)$ converges.}
\begin{proof}
	\pf\ Root Test.
\end{proof}
\step{4}{If $|z| > R$ then $\sum_{n=0}^\infty a_n(z)$ diverges.}
\begin{proof}
	\pf\ Root Test.
\end{proof}
\qed
...
\begin{proof}
\pf
\step{1}{\pick\ $M$ such that, for all $N$, we have $\| \sum_{n=0}^N a_n \| \leq M$.}
\step{2}{\pflet{$\epsilon > 0$}}
\step{3}{\pick\ $N$ such that $b_N \leq \epsilon / 2M$.}
\step{4}{\pflet{$N \leq p \leq q$}}
\step{4a}{For any integer $k$, \pflet{$A_k = \sum_{n=0}^k a_n$.}}
\step{5}{$\| \sum_{n=p}^q a_n b_n \| \leq \epsilon$}
\begin{proof}
	\pf
	\begin{align*}
...
\begin{proof}
\pf
\step{1}{\pflet{$z$ be a complex number with $|z| = 1$ and $z \neq 1$.}}
\step{2}{For $n \in \mathbb{N}$, \pflet{$a_n = z^n$.}}
\step{3}{For $n \in \mathbb{N}$, \pflet{$b_n = c_n$.}}
\step{4}{The partial sums $\sum_{n=0}^N a_n$ form a bounded sequence.}
\begin{proof}
\pf
\begin{align*}
...
\end{align*}
\end{proof}
\step{5}{$(b_n)$ is monotone decreasing with limit 0.}
\qedstep
\begin{proof}
...
\pf
\step{1}{\pflet{$\epsilon > 0$}}
\step{2}{\pick\ $N$ such that, for all $p,q \geq N$, we have
\[ \sum_{n=p}^q \|a_n\| \leq \epsilon \enspace . \]}
\step{3}{For $p,q \geq N$, we have
\[ \left\| \sum_{n=p}^q a_n \right\| \leq \epsilon \enspace . \]S}
\qedstep
\begin{proof}
	\pf\ Cauchy criterion.
\end{proof}
...
\begin{proof}
\pf
\step{0}{\pflet{\[ A = \sum_{n=0}^\infty a_n \]}}
\step{00}{\pflet{\[ B = \sum_{n=0}^\infty b_n \]}}
\step{1}{For $n \in \mathbb{N}$, \pflet{\[ A_n = \sum_{k=0}^n a_k \enspace . \]}}
\step{2}{For $n \in \mathbb{N}$, \pflet{\[ B_n = \sum_{k=0}^n b_k \enspace . \]}}
\step{3}{For $n \in \mathbb{N}$, \pflet{\[ C_n = \sum_{k=0}^n a_k b_{n-k} \enspace . \]}}
\step{4}{For $n \in \mathbb{N}$, \pflet{\[ \beta_n = B_n - B\]}}
\step{5}{For $n \in \mathbb{N}$,
\[ C_n = A_n B + \sum_{k=0}^n a_k \beta_{n-k} \enspace . \]}
\step{6}{For $n \in \mathbb{N}$, \pflet{\[ \gamma_n = \sum_{k=0}^n a_k \beta_{n-k} \]}}
\step{7}{$A_n B \rightarrow AB$ as $n \rightarrow \infty$.}
\step{8}{$\gamma_n \rightarrow 0$ as $n \rightarrow \infty$.}
\begin{proof}
	\step{a}{\pflet{$\alpha = \sum_{n=0}^\infty |a_n|$}}
	\step{b}{For all $\epsilon > 0$ we have $\limsup_n |\gamma_n| \leq \epsilon \alpha$.}
	\begin{proof}
		\step{i}{\pflet{$\epsilon > 0$}}
		\step{ii}{\pick\ $N$ such that $\forall n \geq N. |\beta_n| \leq \epsilon$.}
		\step{iii}{For all $n \geq N$ we have $|\gamma_n| \leq \left|\sum_{k=0}^N \beta_k \alpha_{n-k} \right| + \epsilon \alpha$.}
		\begin{proof}
			\pf
			\begin{align*}
			...
			\end{align*}
		\end{proof}
		\step{iv}{\[ \limsup_{n \rightarrow \infty} |\gamma_n| \leq \epsilon \alpha \]}
	\end{proof}
	\step{c}{$\limsup_n \gamma_n = 0$}
\end{proof}
\step{9}{$C_n \rightarrow AB$ as $n \rightarrow \infty$.}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$\sum_n a_n$ and $\sum_n b_n$ be two absolutely convergent series.}}
\step{2}{\pflet{$c_n = \sum_{k=0}^n a_k b_{n-k}$}}
\step{3}{$\sum_n |c_n|$ converges.}
\begin{proof}
	\pf
	\begin{align*}
...
\begin{proof}
\pf
\step{1}{For $n \in \mathbb{Z}^+$, \pflet{\[ p_n = \frac{|a_n| + a_n}{2} \enspace . \]}}
\step{2}{For $n \in \mathbb{Z}^+$, \pflet{\[ q_n = \frac{|a_n| - a_n}{2} \enspace . \]}}
\step{3}{$\forall n \in \mathbb{Z}^+. p_n - q_n = a_n$}
\step{4}{$\forall n \in \mathbb{Z}^+. p_n + q_n = |a_n|$}
\step{5}{$\forall n \in \mathbb{Z}^+. p_n \geq 0$}
\step{6}{$\forall n \in \mathbb{Z}^+. q_n \geq 0$}
\step{7}{$\sum_n p_n$ and $\sum_n q_n$ both diverge.}
\begin{proof}
	\step{a}{It is not the case than $\sum_n p_n$ and $\sum_n q_n$ both converge.}
	\begin{proof}
		\pf\ This would imply that $\sum_n |a_n|$ converges by \stepref{4}.
	\end{proof}
	\step{b}{It is not the case that $\sum_n p_n$ converges and $\sum_n q_n$ diverges.}
	\begin{proof}
		\pf\ This would imply that $\sum_n a_n$ diverges by \stepref{3}.
	\end{proof}
	\step{c}{It is not the case that $\sum_n p_n$ diverges and $\sum_n q_n$ converges.}
	\begin{proof}
		\pf\ This would imply that $\sum_n a_n$ diverges by \stepref{3}.
	\end{proof}
\end{proof}
\step{8}{\pflet{$(P_n)$ be the subsequence of $(a_n)$ consisting of the non-negative terms.}}
\step{9}{\pflet{$(Q_n)$ be the subsequence of $(|a_n|)$ consisting only of the terms such that $a_n$ is negative.}}
\step{10}{$\sum_n P_n$ diverges.}
\begin{proof}
	\pf\ It is the series $\sum_n p_n$ with the zero terms removed.
\end{proof}
\step{11}{$\sum_n Q_n$ diverges.}
\begin{proof}
	\pf\ It is the series $\sum_n q_n$ with the zero terms removed.
\end{proof}
\step{11half}{\pick\ sequences of real numbers $(\alpha_n)$, $(\beta_n)$ such that $\alpha_n \rightarrow \alpha$, $\beta_n \rightarrow \beta$, $\alpha_n < \beta_n$ for all $n$, and $\beta_1 > 0$.}
\step{12}{\pick\ strictly increasing sequences of natural numbers $(m_n)_{n \geq 1}$, $(k_n)_{n \geq 1}$ such that, for all $n$,
\begin{align*}
\sum_{i=1}^{n-1} \left( \sum_{j=m_{i-1}+1}^{m_i} P_j - \sum_{j=k_{i-1}+1}^{k_i} Q_j \right) + \sum_{j=m_{n-1}+1}^{m_n} P_j & > \beta_n \\
\sum_{i=1}^n \left( \sum_{j=m_{i-1}+1}^{m_i} P_j - \sum_{j=k_{i-1}+1}^{k_i} Q_j \right) & < \alpha_n
\end{align*}
and $m_n$ and $k_n$ are the smallest integers that make these inequalities true.}
\begin{proof}
	\pf\ Given the choice of $m_1$, \ldots, $m_n$ and $k_1$, \ldots, $k_n$, there must exist such an $m_{n+1}$ by \stepref{10}, and then there must exist such a $k_{n+1}$ by \stepref{11}.
\end{proof}
\step{13}{For $n \in \mathbb{Z}^+$, \pflet{$x_n = \sum_{i=1}^{n-1} \left( \sum_{j=m_{i-1}+1}^{m_i} P_j - \sum_{j=k_{i-1}+1}^{k_i} Q_j \right) + \sum_{j=m_{n-1}+1}^{m_n} P_j$}}
\step{14}{For $n \in \mathbb{Z}^+$, \pflet{$y_n = \sum_{i=1}^n \left( \sum_{j=m_{i-1}+1}^{m_i} P_j - \sum_{j=k_{i-1}+1}^{k_i} Q_j \right)$}}
\step{15}{For $n \in \mathbb{Z}^+$ we have
\[ | x_n - \beta_n | \leq P_{m_n} \enspace . \]}
\begin{proof}
	\pf\ By minimality of $m_n$.
\end{proof}
\step{16}{For $n \in \mathbb{Z}^+$ we have
\[ | y_n - \alpha_n | \leq Q_{k_n} \enspace . \]}
\begin{proof}
	\pf\ By minimality of $k_n$.
\end{proof}
\step{17}{$P_n \rightarrow 0$ as $n \rightarrow \infty$.}
\begin{proof}
	\pf\ Since $a_n \rightarrow 0$ as $n \rightarrow \infty$.
\end{proof}
\step{18}{$Q_n \rightarrow 0$ as $n \rightarrow \infty$.}
\begin{proof}
	\pf\ Since $a_n \rightarrow 0$ as $n \rightarrow \infty$.
\end{proof}
\step{19}{$x_n \rightarrow \beta$ as $n \rightarrow \infty$.}
\begin{proof}
	\pf\ \stepref{15}, \stepref{17}
\end{proof}
\step{20}{$y_n \rightarrow \alpha$ as $n \rightarrow \infty$.}
\begin{proof}
	\pf\ \stepref{16}, \stepref{18}
\end{proof}
\step{21}{No number less than $\alpha$ or greater than $\beta$ is a subsequential limit of the partial sums of the series $P_1 + \cdots + P_{m_1} -Q_1 - \cdots -Q_{k_1} + P_{m_1 + 1} + \cdots + P_{m_2} -Q_{k_1+1} - \cdots -Q_{k_2} + \cdots$}
\begin{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$\sum_n a_n' = \sum_n a_{k_n}$ be a rearrangement with partial sums $s_n'$.}}
\step{2}{\pflet{$\epsilon > 0$}}
\step{3}{\pick\ $N$ such that, for all $m \geq n \geq N$, we have
\[ \sum_{i=n}^m \|a_i\| \leq \epsilon/3 \enspace . \]}
\step{4}{\pick\ $p$ such that $\{ 1, \ldots, N \} \subseteq \{ k_1, k_2, \ldots, k_p \}$.}
\step{5}{For all $n > p$ we have $\|s_n - s_n'\| \leq \epsilon$.}
\begin{proof}
	\pf
	\begin{align*}
	...
	\end{align*}
\end{proof}
\step{6}{$s_n' \rightarrow s$ as $n \rightarrow \infty$.}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{If $f(x) \rightarrow q$ as $x \rightarrow p$ then, for every sequence $(p_n)$ in $E - \{p\}$ with limit $p$, we have $f(p_n) \rightarrow q$ as $n \rightarrow \infty$.}
\begin{proof}
	\step{a}{\assume{$f(x) \rightarrow q$ as $x \rightarrow p$.}}
	\step{b}{\pflet{$(p_n)$ be a sequence in $E - \{p\}$ with limit $p$.}}
	\step{c}{\pflet{$\epsilon > 0$}}
	\step{d}{\pick\ $\delta > 0$ such that, for all $x \in E$, if $0 < d(x,p) < \delta$ then $d(f(x),q) < \epsilon$.}
	\step{d}{\pick\ $N$ such that, for all $n \geq N$, we have $d(p_n,p) < \delta$}
	\step{e}{$\forall n \geq N. d(f(p_n),q) < \epsilon$}
\end{proof}
\step{2}{If, for every sequence $(p_n)$ in $E - \{p\}$ with limit $p$, we have $f(p_n) \rightarrow q$ as $n \rightarrow \infty$, then $f(x) \rightarrow q$ as $x \rightarrow p$.}
\begin{proof}
	\step{b}{\assume{$f(x) \nrightarrow q$ as $x \rightarrow p$.}}
	\step{c}{\pick\ $\epsilon > 0$ such that, for all $\delta > 0$, there exists a $x \in E$ such that $0 < d(x,p) < \delta$ and $d(f(x),q) \geq \epsilon$.}
	\step{d}{For all $n \in \mathbb{Z}^+$, \pick\ $p_n \in E$ such that $0 < d(p_n,p) < 1/n$ and $d(f(p_n),q) \geq \epsilon$.}
	\step{e}{$p_n \rightarrow p$ as $n \rightarrow \infty$.}
	\step{f}{$f(p_n) \nrightarrow q$ as $n \rightarrow \infty$.}
\end{proof}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$(p_n)$ be a sequence in $E$ that converges to $p$.}}
\step{2}{$f(p_n) \rightarrow a$ as $n \rightarrow \infty$.}
\step{3}{$g(p_n) \rightarrow b$ as $n \rightarrow \infty$.}
\step{4}{$f(p_n) + g(p_n) \rightarrow a + b$ as $n \rightarrow \infty$.}
\begin{proof}
...
\step{2}{$f(p_n) \rightarrow a$ as $n \rightarrow \infty$.}
\step{3}{$g(p_n) \rightarrow b$ as $n \rightarrow \infty$.}
\step{4}{$f(p_n) g(p_n) \rightarrow ab$ as $n \rightarrow \infty$.}
\begin{proof}
...
\step{1}{\pflet{$(p_n)$ be a sequence in $E$ that converges to $p$.}}
\step{2}{$f(p_n) \rightarrow a$ as $n \rightarrow \infty$.}
\step{4}{$f(p_n)^{-1} \rightarrow a^{-1}$ as $n \rightarrow \infty$.}
\begin{proof}
...
\step{2}{$f(p_n) \rightarrow a$ as $n \rightarrow \infty$.}
\step{3}{$g(p_n) \rightarrow b$ as $n \rightarrow \infty$.}
\step{4}{$f(p_n) \cdot g(p_n) \rightarrow a \cdot b$ as $n \rightarrow \infty$.}
\begin{proof}
...
\begin{thm}
Let $X$ be a metric space, $E \subseteq X$ and $p
 \in E$ be a limit point of $E$. Then $f$ is continuous at $p$ iff $f(x) \rightarrow f(p)$ as $x \rightarrow p$.
\end{thm}
...
\begin{thm}
Let $X$, $Y$ and $Z$ be metric spaces. Let $E \subseteq X$. Let $f : E \rightarrow Y$ and $g : f(E) \rightarrow Z$. Let $p \in E$. If $f$ is continuous at $p$ and $g$ is continuous at $f(p)$ then $g \circ f$ is continuous at $p$.
\end{thm}

\begin{proof}
\pf
\step{1}{\pflet{$\epsilon > 0$}}
\step{2}{\pick\ $\delta_1 > 0$ such that, for all $y \in f(E)$, if $d(y,f(p)) < \delta_1$ then $d(g(y),g(f(p))) < \epsilon$.}
\step{3}{\pick\ $\delta_2 > 0$ such that, for all $x \in E$, if $d(x,p) < \delta_2$ then $d(f(x),f(p)) < \delta_1$.}
\step{4}{For all $x \in E$, if $d(x,p) < \delta_2$ then $d(g(f(x)),g(f(p))) < \epsilon$.}
\qed
\end{proof}

\begin{thm}
Let $X$ and $Y$ be metric spaces. Let $f : X \rightarrow Y$. Then $f$ is continuous if and only if, for every open set $V \subseteq Y$, we have $f^{-1}(V)$ is open in $X$.
\end{thm}

\begin{proof}
\pf
\step{1}{If $f$ is continuous then, for every open set $V$ in $Y$, we have $f^{-1}(V)$ is open in $X$.}
\begin{proof}
	\step{a}{\assume{$f$ is continuous.}}
	\step{b}{\pflet{$V$ be an open set in $Y$.} \prove{$f^{-1}(V)$ is open.}}
	\step{c}{\pflet{$x \in f^{-1}(V)$}}
	\step{d}{\pick\ $\epsilon > 0$ such that $B(f(x),\epsilon) \subseteq V$.}
	\step{e}{\pick\ $\delta > 0$ such that, for all $x' \in X$, if $d(x',x) < \delta$ then $d(f(x'),f(x)) < \epsilon$.}
	\step{f}{$B(x,\delta) \subseteq f^{-1}(V)$}
\end{proof}
\step{2}{If, for every open set $V$ in $Y$, we have $f^{-1}(V)$ is open in $X$, then $f$ is continuous.}
\begin{proof}
	\step{a}{\assume{For every open set $V$ in $Y$, we have $f^{-1}(V)$ is open in $X$.}}
	\step{b}{\pflet{$p \in X$}}
	\step{c}{\pflet{$\epsilon > 0$}}
	\step{d}{$f^{-1}(B(f(p),\epsilon))$ is open in $X$.}
	\step{e}{\pick\ $\delta > 0$ such that $B(p,\delta) \subseteq f^{-1}(B(f(p), \epsilon))$.}
	\step{f}{For all $x \in X$, if $d(x,p) < \delta$ then $d(f(x),f(p)) < \epsilon$.}
\end{proof}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{Each $\pi_i$ is continuous.}
\begin{proof}
	\step{a}{\pflet{$\vec{p} \in \mathbb{R}^k$}}
	\step{b}{\pflet{$\epsilon > 0$}}
	\step{c}{\pflet{$\vec{q} \in \mathbb{R}^k$}}
	\step{d}{\assume{$\| \vec{p} - \vec{q} \| < \epsilon$}}
	\step{e}{$|p_i - q_i| < \epsilon$}
\end{proof}
\step{2}{If, for all $i$, we have $\pi_i \circ f$ is continuous, then $f$ is continuous.}
\begin{proof}
	\step{a}{\assume{For all $i$, we have $\pi_i \circ f$ is continuous.}}
	\step{b}{\pflet{$p \in X$}}
	\step{c}{\pflet{$\epsilon > 0$}}
	\step{d}{For $i = 1, \ldots, k$, \pick\ $\delta_i > 0$ such that, for all $x \in X$, we have if $d(x,p) < \delta_i$ then $|\pi_i(f(p)) - \pi_i(f(x))| < \epsilon / \sqrt{k}$}
	\step{e}{\pflet{$\delta = \min(\delta_1, \ldots, \delta_k)$}}
	\step{f}{\pflet{$q \in X$ with $d(p,q) < \delta$.}}
	\step{g}{$\| f(p) - f(q) \| < \epsilon$}
	\begin{proof}
		\pf
		\begin{align*}
...
\begin{proof}
\pf
\step{1}{\pflet{$\mathcal{V}$ be an open cover of $f(X)$.}}
\step{2}{$\{ f^{-1}(V) : V \in \mathcal{V} \}$ is an open cover of $X$.}
\step{3}{\pick\ a finite subcover $\{ f^{-1}(V_1), \ldots, f^{-1}(V_n) \}$.}
\step{4}{$\{ V_1, \ldots, V_n \}$ covers $Y$.}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\case{$E$ is bounded.}}
\begin{proof}
	\step{a}{\pick\ a limit point $x_0$ of $E$ that is not in $E$.}
	\step{b}{Define $f : E \rightarrow \mathbb{R}$ by $f(x) = 1/(x - x_0)$.}
	\step{c}{$f$ is continuous and unbounded.}
\end{proof}
\step{2}{\case{$E$ is unbounded.}}
\begin{proof}
...
\step{1}{\case{$E$ is bounded.}}
\begin{proof}
	\step{a}{\pick\ a limit point $x_0$ for $E$ such that $x_0 \notin E$.}
	\step{b}{Define $g : E \rightarrow \mathbb{R}$ by $g(x) = 1/(1 + (x - x_0)^2)$.}
	\step{c}{$g$ is continuous and bounded but does not attain its supremum 1.}
\end{proof}
\step{2}{\case{$E$ is unbounded.}}
\begin{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$V$ be open in $X$.}}
\step{2}{$X - V$ is compact.}
\step{3}{$f(X - V)$ is compact.}
\step{4}{$Y - f(V)$ is compact.}
\step{5}{$Y - f(V)$ is closed.}
\step{6}{$f(V)$ is open.}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$X$ be a connected metric space and $Y$ a metric space.}}
\step{2}{\pflet{$f : X \twoheadrightarrow Y$ be a continuous surjection.}}
\step{3}{\assume{for a contradiction $A$ and $B$ form a separation of $Y$.}}
\step{4}{$f^{-1}(A)$ and $f^{-1}(B)$ form a separation of $X$.}
\qedstep
\begin{proof}
	\pf\ This contradicts \stepref{1}.
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$\epsilon > 0$ and $A \in \Omega$}}
\step{1a}{\pflet{$\alpha = 1/\| A^{-1} \|$}}
\step{2}{\pflet{$\delta = \alpha^2 \epsilon / (1 + \alpha \epsilon)$}}
\step{3}{\pflet{$B \in \Omega$ with $\| B - A \| < \delta$.}}
\step{3a}{$\| B^{-1} \| \leq (\alpha - \delta)^{-1}$}
\begin{proof}
	\step{i}{For all $\vec{y} \in \mathbb{R}^n$ we have $(\alpha - \delta) \| B^{-1} \vec{y} \| \leq \| \vec{y} \|$.}
	\begin{proof}
		\pf
		\begin{align*}
...
\begin{proof}
\pf
\step{1}{If $f(x) \rightarrow q$ as $x \rightarrow c$ then $f(x) \rightarrow q$ as $x \rightarrow c-$ and $f(x) \rightarrow q$ as $x \rightarrow c+$.}
\begin{proof}
	\pf\ Theorem \ref{thm:limits_and_sequences}.
\end{proof}
\step{2}{If $f(x) \rightarrow q$ as $x \rightarrow c-$ and $f(x) \rightarrow q$ as $x \rightarrow c+$ then $f(x) \rightarrow q$ as $x \rightarrow c$.}
\begin{proof}
	\step{a}{\assume{$f(x) \rightarrow q$ as $x \rightarrow c-$ and $f(x) \rightarrow q$ as $x \rightarrow c+$.}}
	\step{b}{\assume{for a contradiction $f(x) \nrightarrow q$ as $x \rightarrow c$.}}
	\step{c}{\pick\ a sequence $(p_n)$ such that $p_n \rightarrow c$ as $n \rightarrow \infty$, $f(p_n) \nrightarrow q$ as $n \rightarrow \infty$, and $p_n \neq c$ for all $n$.}
	\step{d}{\case{There are only finitely many $n$ such that $p_n > c$.}}
	\begin{proof}
		\step{i}{\pflet{$(q_n)$ be the subsequence of $(p_n)$ consisting of all the terms such that $p_n < c$.}}
		\step{ii}{$q_n \rightarrow c$ as $n \rightarrow \infty$.}
		\step{iii}{$f(q_n) \nrightarrow q$ as $n \rightarrow \infty$.}
		\qedstep
		\begin{proof}
			\pf\ This contradicts \stepref{a}.
		\end{proof}
	\end{proof}
	\step{e}{\case{There are only finitely many $n$ such that $p_n < c$.}}
	\begin{proof}
		\pf\ Similar.
	\end{proof}
	\step{f}{\case{There are infinitely many $n$ such that $p_n > c$ and infinitely many $n$ such that $p_n < c$.}}
	\begin{proof}
		\step{i}{\pflet{$(q_n)$ the subsequence of $(p_n)$ consisting of all the terms such that $p_n > c$, and $(r_n)$ the subsequence consisting of all the terms such that $p_n < c$.}}
		\step{ii}{$q_n \rightarrow c$ as $n \rightarrow \infty$ and $r_n \rightarrow c$ as $n \rightarrow \infty$.}
		\step{iii}{It is not the case that $f(q_n) \rightarrow q$ as $n \rightarrow \infty$ and $f(r_n) \rightarrow q$ as $n \rightarrow \infty$.}
		\begin{proof}
			\pf\ If $f(q_n) \rightarrow q$ as $n \rightarrow \infty$ and $f(r_n) \rightarrow q$ as $n \rightarrow \infty$ then $f(p_n) \rightarrow q$ as $n \rightarrow \infty$.
		\end{proof}
		\qedstep
		\begin{proof}
			\pf\ This contradicts \stepref{a}.
...
\begin{proof}
\pf
\step{1}{\assume{w.l.o.g. $f$ is monotonically increasing on $(a,b)$.}}
\step{2}{$f(x) \rightarrow \sup_{a < x < c} f(x)$ as $x \rightarrow c-$.}
\begin{proof}
	\step{a}{\pflet{$(t_n)$ be a sequence in $(a,c)$ such that $t_n \rightarrow c$ as $n \rightarrow \infty$.} \prove{$f(t_n) \rightarrow \sup_{a < x < c} f(x)$ as $n \rightarrow \infty$.}}
	\step{b}{\pflet{$\epsilon > 0$}}
	\step{c}{\pick\ $x \in (a,c)$ such that $f(x)$}
	%TODO
\end{proof}
\step{3}{$f(x) \rightarrow \inf_{c < x < b} f(x)$ as $x \rightarrow c+$.}
\begin{proof}
	\pf\ Similar.
\end{proof}
\qed
...
\pf
\step{1}{\pflet{$\epsilon > 0$}}
\step{2}{For all $p \in X$, \pick\ $\phi(p) > 0$ such that, for all $q \in X$, if $d(p,q) < \phi(x)$ then $d(f(p),f(q)) < \epsilon / 2$.}
\step{3}{For all $p \in X$, \pflet{$J(p) = B(p, \phi(x)/2)$.}}
\step{4}{$\{ J(p) : p \in X \}$ is an open cover of $X$.}
\step{5}{\pick\ a finite subcover $\{ J(p_1), \ldots, J(p_n) \}$.}
\step{6}{\pflet{$\delta = \min(\phi(p_1), \ldots, \phi(p_n)) / 2$}}
\step{7}{\pflet{$p,q \in X$ with $d(p,q) < \delta$.}}
\step{8}{\pick\ $m$ such that $p \in J(p_m)$.}
\step{9}{$d(p,p_m) < \phi(p_m) / 2$}
\step{10}{$d(q,p_m) < \phi(p_m)$}
\step{11}{$d(f(p),f(q)) < \epsilon$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\assume{$A_1$ and $A_2$ are derivatives of $f$ at $\vec{x}$.}}
\step{2}{\pflet{$B = A_1 - A_2$}}
\step{3}{For all $\vec{h}$ such that $\vec{x} + \vec{h} \in E$ we have
\[ \| B (\vec{h}) \| \leq \| f(\vec{x} + \vec{h}) - f(\vec{x}) - A_1(\vec{h}) \| + \| f(\vec{x} + \vec{h}) - f(\vec{x}) - A_2(\vec{h}) \| \]}
\step{4}{$\| B (\vec{h}) \| / \| \vec{h} \| \rightarrow 0$ as $\vec{h} \rightarrow \vec{0}$.}
\step{5}{For $\vec{h} \neq \vec{0}$ such that $\vec{x} + \vec{h} \in E$ we have
\[ \frac{\| B(t \vec{h}) \|}{\|t \vec{h}\|} \rightarrow 0 \text{ as } t \rightarrow 0 \enspace . \]}
\step{6}{For $\vec{h} \neq \vec{0}$ such that $\vec{x} + \vec{h} \in E$ we have
\[ \frac{\| B (\vec{h})\|}{\| \vec{h}\|} \rightarrow 0 \text{ as } t \rightarrow 0 \enspace . \]}
\begin{proof}
	\pf\ Since $B$ is linear.
\end{proof}
\step{7}{For $\vec{h} \neq \vec{0}$ such that $\vec{x} + \vec{h} \in E$ we have $B \vec{h} = \vec{0}$}
\step{8}{$B = 0$}
\step{9}{$A_1 = A_2$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$\vec{y_0} = f(\vec{x_0})$}}
\step{2}{\pflet{$A = f'(\vec{x_0})$}}
\step{3}{\pflet{$B = g'(\vec{y_0})$}}
\step{4}{For $\vec{h}$ such that $\vec{x_0} + \vec{h} \in E$, \pflet{$u(\vec{h}) = f(\vec{x_0} + \vec{h}) - f(\vec{x_0}) + A(\vec{h})$ \enspace .}}
\step{5}{For $\vec{k}$ such that $\vec{y_0} + \vec{k} \in U$, \pflet{$v(\vec{k}) = g(\vec{y_0} + \vec{k}) - g(\vec{y_0}) + B(\vec{k})$ \enspace . }}
\step{6}{For $\vec{h}$ non-zero such that $\vec{x_0} + \vec{h} \in E$, \pflet{$\epsilon(\vec{h}) = \| u(\vec{h})\| / \|\vec{h}\|$ \enspace .}}
\step{7}{For $\vec{k}$ non-zero such that $\vec{y_0} + \vec{k} \in U$, \pflet{$\eta(\vec{k}) = \| v(\vec{k}) \| / \| \vec{k} \|$ \enspace . }}
\step{8}{$\epsilon(\vec{h}) \rightarrow 0$ as $\vec{h} \rightarrow \vec{0}$}
\begin{proof}
	\pf\ \stepref{2}, \stepref{4}, \stepref{6}.
\end{proof}
\step{9}{$\eta(\vec{k}) \rightarrow 0$ as $\vec{k} \rightarrow \vec{0}$}
\begin{proof}
	\pf\ \stepref{3}, \stepref{5}, \stepref{7}
\end{proof}
\step{10}{For $\vec{h}$ such that $\vec{x_0} + \vec{h} \in E$, \pflet{$k(\vec{h}) = f(\vec{x_0} + \vec{h}) - f(\vec{x_0})$ \enspace . }}
%TODO
\end{proof}
