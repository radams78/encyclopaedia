...
\begin{cor}
\label{cor:timesZcancel}
For any integers $a$, $b$ and $C$, if $ac = bc$ and $c \neq 0$ then $a = b$.
\end{cor}

\section{The Rationals}

\begin{df}[Rational Numbers]
The set $\mathbb{Q}$ of \emph{rational numbers} is the quotient set $(\mathbb{Z} \times (\mathbb{Z} - \{0\})) / \sim$, where $(a,b) \sim (c,d)$ iff $ad = bc$.

We prove this is an equivalence relation on $\mathbb{Z} \times (\mathbb{Z} - \{0\})$.
\end{df}
...
\begin{proof}
	...
	\step{d}{$af = be$}
	\begin{proof}
		\pf\ Corollary \ref{cor:timesZcancel}.
	\end{proof}
\end{proof}
\qed
\end{proof}
...
\begin{df}
Define \emph{addition} $+$ on $\mathbb{Q}$ by $[(a,b)] + [(c,d)] = [(ad + bc, bd)]$.
...
\step{1}{If $b \neq 0$ and $d \neq 0$ then $bd \neq 0$.}
\begin{proof}
	\pf\ Proposition \ref{prop:Zno_zero_div}.
\end{proof}
