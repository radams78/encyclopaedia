...
\begin{df}[Injective]
A function $\mathbf{F} : \mathbf{A} \rightarrow \mathbf{B}$ is \emph{one-to-one}, \emph{injective} or an \emph{injection} iff, for all $x,y \in \mathbf{A}$, if $\mathbf{F}(x) = \mathbf{F}(y)$, then $x = y$.
\end{df}
...
\begin{df}[Dependent Product Set]
...
\begin{props}
Let $\mathbf{H}(i)$ be a class with a parameter. Then the following is a theorem:

Let $I$ be a set. Assume that, for all $i \in I$, the class $\mathbf{H}(i)$ is a set. Then $\prod_{i \in I} \mathbf{H}(i)$ is a set.
\end{props}
