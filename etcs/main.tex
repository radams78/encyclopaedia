...
\end{proof}
\step{2}{If, for all $x,y \in A$, if $f(x) = f(y)$ then $x = y$, then $f$ is injective.}
\begin{proof}
	\step{a}{\assume{For all $x,y \in A$, if $f(x) = f(y)$ then $x = y$.}}
	\step{b}{\pflet{$X$ be a set and $x,y : X \rightarrow A$}}
	\step{c}{\assume{$f \circ x = f \circ y$}}
	\step{d}{For all $t \in X$ we have $x(t) = y(t)$.}
	\begin{proof}
		\step{i}{\pflet{$t \in X$}}
		\step{ii}{$f(x(t)) = f(y(t))$}
		\begin{proof}
