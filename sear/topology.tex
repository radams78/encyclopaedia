...
Let $X$ be a set and $\mathcal{T} \subseteq \mathcal{P} X$. Then we say $(X, \mathcal{T})$ is a \emph{topological space} iff:
\begin{itemize}
\item For any $\mathcal{U} \subseteq \mathcal{T}$ we have $\bigcup \mathcal{U} \in \mathcal{T}$.
\item For any $U, V \in \mathcal{T}$ we have $U \cap V \in \mathcal{T}$.
\item $X \in \mathcal{T}$
\end{itemize}
We call $\mathcal{T}$ the \emph{topology} of the toplogical space, and call its elements \emph{open} sets. We shall often write $X$ for the topological space $(X, \mathcal{T})$.
\end{df}
...
\begin{proof}
\pf
\step{1}{In any topology on $X$ we have $\emptyset$ is closed.}
\begin{proof}
	\pf\ Since $X - \emptyset = X$ is open.
\end{proof}
\step{2}{In any topology on $X$, the intersection of a set $\mathcal{A}$ of closed sets is closed.}
\begin{proof}
	\pf\ Since $X - \bigcap \mathcal{A} = \bigcup_{A \in \mathcal{A}} (X - A)$ is open.
\end{proof}
\step{3}{In any topology on $X$, the union of two closed sets is closed.}
\begin{proof}
	\pf\ For any closed sets $C$ and $D$, we have $X - (C \cup D) = (X - C) \cap (X - D)$ is open.
\end{proof}
\step{4}{If $\mathcal{C}$ is a set satisfying 1--3, then $\{ X - C : C \in \mathcal{C} \}$ is a topology on $X$ with respect to which $\mathcal{C}$ is the set of closed sets.}
\begin{proof}
	\step{a}{\pflet{$\mathcal{C}$ be a set satisfying 1--3.}}
	\step{b}{\pflet{$\mathcal{T} = \{ X - C : C \in \mathcal{C} \}$}}
	\step{c}{For all $U \in \mathcal{T}$ we have $X - U \in \mathcal{C}$.}
	\step{c}{$\mathcal{T}$ is a topology on $X$.}
	\begin{proof}
		\step{i}{For all $\mathcal{U} \subseteq \mathcal{T}$ we have $\bigcup \mathcal{U} \in \mathcal{T}$.}
		\begin{proof}
			\step{one}{\pflet{$\mathcal{U} \subseteq \mathcal{T}$}}
			\step{two}{For all $U \in \mathcal{U}$ we have $X - U \in \mathcal{C}$.}
			\step{two}{$X - \bigcup \mathcal{U} \in \mathcal{C}$}
			\begin{proof}
				\pf
				\begin{align*}
					X - \bigcup \mathcal{U} & = \bigcap_{U \in \mathcal{U}} (X - U) \\
					& \in \mathcal{C}
				\end{align*}
			\end{proof}
			\step{three}{$\bigcup \mathcal{U} \in \mathcal{T}$}
		\end{proof}
		\step{ii}{For all $U,V \in \mathcal{T}$ we have $U \cap V \in \mathcal{T}$.}
		\step{iii}{$X \in \mathcal{T}$}
	\end{proof}
	\step{d}{For any set $C$ we have $C \in \mathcal{C}$ iff $C$ is closed with respect to $\mathcal{T}$.}
\end{proof}
\step{5}{If $\mathcal{T}$ is any topology on $X$ then $\mathcal{T} = \{ X - C : C \text{ is closed in } \mathcal{T} \}$.}
\begin{proof}
	\pf\ Proposition \ref{prop:open_closed}.
\end{proof}
\qed
\end{proof}
...
\begin{prop}
A set $C$ is closed if and only if $C = \overline{C}$.
\end{prop}
...
Let $X$ be a topological space. Let $a \in X$. Then $a$ is an \emph{isolated point} iff $\{a\}$ is open.
\end{df}
...
\begin{prop}
\label{prop:closed_in_closed}
Let $X$ be a topological space. Let $Y$ be a subspace of $X$. Let $C \subseteq Y$. If $Y$ is closed in $X$ and $C$ is closed in $Y$ then $C$ is closed in $X$.
\end{prop}
...
Let $X$ be a Hausdorff space. Let $Y$ be a compact subspace. Let $x \in X - Y$. Then there exist disjoint open sets $U$ and $V$ such that $x \in U$ and $Y \subseteq V$.
\end{lm}
...
\begin{prop}
\label{prop:separate_compact}
Let $X$ be a Hausdorff space. Let $A$ and $B$ be disjoint compact subspaces of $X$. Then there exist disjoint open sets $U$ and $V$ with $A \subseteq U$ and $B \subseteq V$.
\end{prop}

\begin{proof}
\pf
\step{1}{For all $x \in A$, there exist disjoint open sets $U'$ and $V'$ with $x \in U'$ and $B \subseteq V'$.}
\begin{proof}
	\pf\ Lemma \ref{lm:compact_subspace}.
\end{proof}
\step{2}{$\{ U' \text{ open in } X : \exists V' \text{ open in }X. U' \cap V' = \emptyset \wedge B \subseteq V' \}$ is a set of open sets that covers $A$.}
\step{3}{\pick\ a finite subset $\{ U_1, \ldots, U_n \}$ that covers $A$.}
\begin{proof}
	\pf\ Lemma \ref{lm:subspace_compact}.
\end{proof}
\step{4}{For $i = 1, \ldots, n$, \pick\ $V_i$ open in $X$ such that $U_i \cap V_i = \emptyset$ and $B \subseteq V_i$.}
\step{5}{\pflet{$U = U_1 \cup \cdots \cup U_n$}}
\step{6}{\pflet{$V = V_1 \cap \cdots \cap V_n$}}
\step{7}{$U$ and $V$ are disjoint and open.}
\step{8}{$A \subseteq U$}
\step{9}{$B \subseteq V$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$X$ be a compact space and $f : X \twoheadrightarrow Y$ be a surjective continuous function.}}
\step{2}{\pflet{$\mathcal{V}$ be an open cover of $Y$.}}
\step{3}{$\{ \inv{f}(V) : V \in \mathcal{V} \}$ is an open cover of $U$.}
\step{4}{\pick\ a finite subcover $\{ \inv{f}(V_1), \ldots, \inv{f}(V_n) \}$.}
...
\begin{proof}
\pf
\step{1}{\pflet{$f : X \rightarrow Y$ be continuous.}}
\step{2}{\pflet{$C$ be a closed set in $X$.}}
\step{3}{$C$ is compact.}
\begin{proof}
	\pf\ Theorem \ref{thm:closed_compact}.
\end{proof}
\step{4}{$f(C)$ is compact.}
\begin{proof}
	\pf\ Theorem \ref{thm:compact_image}.
\end{proof}
\step{5}{$f(C)$ is closed in $Y$.}
\begin{proof}
...
\begin{thm}
Let $X$ and $Y$ be topological spaces. Let $A$ be a compact subspace of $X$ and $B$ a compact subspace of $Y$. Let $N$ be an open set that includes $A \times B$. Then there exists open sets $U$ in $X$ and $V$ in $Y$ such that
\[ A \times B \subseteq U \times V \subseteq N \enspace . \]
\end{thm}

\begin{proof}
\pf
\step{1}{For all $x \in A$, there exist open sets $U$ in $X$ and $V$ in $Y$ such that $x \in U$, $B \subseteq V$, and $U \times V \subseteq N$.}
\begin{proof}
	\step{a}{\pflet{$x \in A$}}
	\step{b}{For all $y \in B$, there exist neighbourhoods $U$ of $x$ and $V$ of $y$ such that $U \times V \subseteq N$}
	\step{c}{\pick\ open sets $V_1$, \ldots, $V_n$ that cover $B$ such that, for $i=1, \ldots, n$, there exists a neighbourhood $U_i$ of $x$ such that $U_i \times V_i \subseteq N$}
	\step{d}{\pflet{$U = U_1 \cap \cdots \cap U_n$}}
	\step{e}{\pflet{$V = V_1 \cup \cdots \cup V_n$}}
	\step{f}{$U$ is open in $X$.}
	\step{g}{$V$ is open in $Y$.}
	\step{h}{$x \in U$}
	\step{i}{$B \subseteq V$}
	\step{j}{$U \times V \subseteq N$}
\end{proof}
\step{2}{\pick\ open sets $U_1$, \ldots, $U_n$ in $X$ that cover $A$ such that, for $i = 1, \ldots, n$, there exists $V_i$ open in $Y$ such that $B \subseteq V_i$ and $U_i \times V_i \subseteq N$.}
\step{3}{\pflet{$U = U_1 \cup \cdots \cup U_n$}}
\step{4}{\pflet{$V = V_1 \cap \cdots \cap V_n$}}
\step{5}{$U$ is open in $X$.}
\step{6}{$V$ is open in $Y$.}
\step{7}{$A \times B \subseteq U \times V \subseteq N$}
\qed
\end{proof}

\begin{cor}[Tube Lemma]
...
\begin{proof}
\pf
\step{1}{\pflet{$X$ and $Y$ be compact spaces.}}
\step{3}{\pflet{$\mathcal{A}$ be an open covering of $X \times Y$.}}
\step{4}{For all $x \in X$, there exists a neighbourhood $W$ of $x$ such that $W \times Y$ can be covered by finitely many elements of $\mathcal{A}$.}
\begin{proof}
	\step{a}{\pflet{$x \in X$}}
	\step{b}{$\{ x \} \times Y$ is compact.}
	\begin{proof}
		\pf\ It is homeomorphic to $Y$.
	\end{proof}
	\step{c}{\pick\ a finite subcollection $\{ A_1, \ldots, A_n \}$ of $\mathcal{A}$ that covers $\{x\} \times Y$.}
	\step{d}{There exists a neighbourhood $W$ of $x$ such that $W \times Y \subseteq A_1 \cup \cdots \cup A_n$.}
	\begin{proof}
		\pf\ Tube Lemma
	\end{proof}
\end{proof}
\step{5}{\pick\ finitely many open sets $W_1$, \ldots, $W_n$ that cover $X$ such that each $W_i \times Y$ can be covered by finitely many elements of $\mathcal{A}$.}
\step{6}{For $i = 1, \ldots, n$, \pick\ a finite subset $\mathcal{A}_i$ of $\mathcal{A}$ that covers $W_i \times Y$.}
\step{7}{$\mathcal{A}_1 \cup \cdots \cup \mathcal{A}_n$ covers $X \times Y$.}
\qed
\end{proof}

\begin{thm}
\label{thm:finite_intersection_property_intersection}
...
\begin{cor}
\label{cor:nested_sequence_of_closed_sets}
...
\begin{proof}
\pf
\step{1}{\pflet{$\mathcal{U}$ be a set of open sets that covers $Y \cup Z$.}}
\step{2}{\pick\ finite subsets $\mathcal{U}_1$ that covers $Y$ and $\mathcal{U}_2$ that covers $Z$.}
\step{3}{$\mathcal{U}_1 \cup \mathcal{U}_2$ is a finite subset of $\mathcal{U}$ that covers $Y \cup Z$.}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{\pflet{$C$ be closed in $X \times Y$.}}
\step{2}{\pflet{$x \in X - \pi_1(C)$}}
\step{3}{$\{x\} \times Y \subseteq (X \times Y) - C$}
\step{4}{\pick\ a neighbourhood $U$ of $x$ such that $U \times Y \subseteq (X \times Y) - C$}
\begin{proof}
	\pf\ Tube Lemma.
\end{proof}
\step{5}{$x \in U \subseteq X - \pi_1(C)$}
\qed
\end{proof}
...
\begin{proof}
\pf
\step{1}{If $f$ is continuous then $G_f$ is closed in $X \times Y$.}
\begin{proof}
	\step{a}{\assume{$f$ is continuous.}}
	\step{b}{\pflet{$(x,y) \in (X \times Y) - G_f$}}
	\step{c}{\pick\ disjoint open neighbourhoods $U$ of $y$ and $V$ of $f(x)$.}
	\step{d}{$(x,y) \in (\inv{f}(V) \times U) \subseteq (X \times Y) - G_f$}
\end{proof}
\step{2}{If $G_f$ is closed in $X \times Y$ then $f$ is continuous.}
\begin{proof}
	\step{a}{\assume{$G_f$ is closed in $X \times Y$.}}
	\step{b}{\pflet{$x_0 \in X$}}
	\step{c}{\pflet{$V$ be a neighbourhood of $f(x_0)$.}}
	\step{d}{$G_f \cap (X \times (Y - V))$ is closed.}
	\step{e}{$\pi_1(G_f \cap (X \times (Y - V)))$ is closed.}
	\begin{proof}
		\pf\ Proposition \ref{prop:projection_closed}.
	\end{proof}
	\step{f}{\pflet{$U = X - \pi_1(G_f \cap (X \times (Y - V)))$}}
	\step{g}{$x_0 \in U$}
	\step{h}{$f(U) \subseteq V$}
\end{proof}
\qed
\end{proof}

\begin{thm}
Let $X$ be a compact Hausdorff space. Let $\mathcal{A}$ be a set of closed connected subspaces of $X$ that is linearly ordered under inclusion. Then $\bigcap \mathcal{A}$ is connected.
\end{thm}

\begin{proof}
\pf
\step{1}{\assume{for a contradiction $(C,D)$ is a separation of $\bigcap \mathcal{A}$.}}
\step{2}{\pick\ disjoint open sets $U$ and $V$ such that $C \subseteq U$ and $D \subseteq V$.}
\begin{proof}
	\step{a}{$C$ and $D$ are closed in $X$.}
	\begin{proof}
		\pf\ Proposition \ref{prop:closed_in_closed}.
	\end{proof}
	\step{b}{$C$ and $D$ are compact.}
	\begin{proof}
		\pf\ Theorem \ref{thm:closed_compact}.
	\end{proof}
	\qedstep
	\begin{proof}
		\pf\ Proposition \ref{prop:separate_compact}.
	\end{proof}
\end{proof}
\step{3}{$\bigcap_{A \in \mathcal{A}}(A - (U \cup V))$ is nonempty.}
\begin{proof}
	\step{a}{$\{ A - (U \cap V) : A \in \mathcal{A} \}$ has the finite intersection property.}
	\begin{proof}
		\step{i}{\pflet{$A_1, \ldots, A_n \in \mathcal{A}$}}
		\step{ii}{\assume{w.l.o.g. $A_1 \supseteq \cdots \supseteq A_n$}}
		\begin{proof}
			\pf\ $\mathcal{A}$ is linearly ordered by inclusion.
		\end{proof}
		\step{iii}{$\bigcap_{i=1}^n (A_i - (U \cup V)) = A_n - (U \cup V)$}
		\step{iv}{$A_n - (U \cup V) \neq \emptyset$}
		\begin{proof}
			\pf\ Otherwise $(A_n \cap U, A_n \cap V)$ would be a separation of $A_n$.
		\end{proof}
	\end{proof}
	\qedstep
	\begin{proof}
		\pf\ Theorem \ref{thm:finite_intersection_property_intersection}.
	\end{proof}
\end{proof}
\qedstep
\begin{proof}
...
\begin{thm}
Let $X$ be a linearly ordered set with the least upper bound property under the order topology. Every closed interval in $X$ is compact.
\end{thm}

\begin{proof}
\pf
\step{1}{\pflet{$a,b \in X$ with $a < b$.}}
\step{2}{\pflet{$\mathcal{A}$ be an open covering of $[a,b]$}}
\step{3}{For any $x \in [a,b)$, there exists $y \in (x,b]$ such that $[x,y]$ can be covered by at most two elements of $\mathcal{A}$.}
\begin{proof}
	\step{a}{\pflet{$x \in [a,b)$}}
	\step{b}{\pick\ $U \in \mathcal{A}$ with $x \in U$.}
	\step{c}{\pick\ $y > x$ such that $[x,y) \subseteq U$.}
	\step{d}{\pick\ $V \in \mathcal{A}$ such that $y \in V$.}
	\step{e}{$[x,y]$ can be covered by $U$ and $V$.}
\end{proof}
\step{4}{\pflet{$C = \{ y \in (a,b] : [a,y] \text{ can be covered by finitely many elements of } \mathcal{A} \}$}}
\step{5}{$C \neq \emptyset$}
\begin{proof}
	\pf\ By \stepref{3}, there exists $y \in (a,b]$ such that $[a,y]$ can be covered by at most two elements of $\mathcal{A}$.
\end{proof}
\step{6}{\pflet{$c = \sup C$}}
\step{7}{$c \in C$}
\begin{proof}
	\step{a}{\pick\ $U \in \mathcal{A}$ such that $c \in U$.}
	\step{b}{\pick\ $y < c$ such that $(y,c] \subseteq U$.}
	\step{c}{\pick\ $z \in C$ such that $y < z$}
	\step{d}{\pick\ a finite $\mathcal{A}_0 \subseteq \mathcal{A}$ that covers $[a,z]$.}
	\step{e}{$\mathcal{A}_0 \cup \{U\}$ covers $[a,c]$.}
	\step{f}{$c \in C$}
\end{proof}
\step{8}{$c = b$}
\begin{proof}
	\step{a}{\assume{for a contradiction $c < b$.}}
	\step{b}{\pick\ $y > c$ such that $[c,y]$ can be covered by at most two elements of $\mathcal{A}$.}
	\step{c}{$[a,c]$ can be covered by finitely many elements of $\mathcal{A}$.}
	\begin{proof}
		\pf\ \stepref{7}
	\end{proof}
	\step{d}{$[a,y]$ can be covered by finitely many elements of $\mathcal{A}$.}
	\step{e}{$y \in C$}
	\qedstep
	\begin{proof}
		\pf\ This is a contradiction.
	\end{proof}
\end{proof}
\step{9}{$[a,b]$ can be covered by finitely many elements of $\mathcal{A}$.}
\qed
\end{proof}
...
\begin{prop}
A linearly ordered set that is compact under the order topology has a greatest and a least element.
\end{prop}

\begin{proof}
\pf
\step{1}{\pflet{$X$ be a linearly ordered set that is compact under the order topology.}}
\step{2}{$X$ has a greatest element.}
\begin{proof}
	\step{a}{\assume{for a contradiction $X$ has no greatest element.}}
	\step{b}{The open rays $(-\infty, a)$ form an open cover of $X$.}
	\step{c}{\pick\ a finite subcover $\{(-\infty, a_1), \ldots, (-\infty, a_n)\}$ where $a_1 \leq \cdots \leq a_n$}
	\step{d}{$a_n < a_n$}
	\qedstep
	\begin{proof}
		\pf\ This is a contradiction.
...
Let $X$ be a compact space and $Y$ a linearly ordered set in the order topology. Let $f : X \rightarrow Y$ be continuous. Then there exist $c,d \in X$ such that, for all $x \in X$, we have $f(c) \leq f(x) \leq f(d)$.
\end{cor}

\begin{proof}
\pf\ Since $f(X)$ is compact, and so has a greatest and least element. \qed
\end{proof}

\section{Perfect Maps}

\begin{df}[Perfect Map]
Let $X$ and $Y$ be topological spaces. A \emph{perfect map} from $X$ to $Y$ is a closed continuous surjective map $p : X \twoheadrightarrow Y$ such that, for all $y \in Y$, we have $\inv{p}(y)$ is compact.
\end{df}

\begin{prop}
Let $X$ be a topological space and $Y$ a compact space. If there exists a perfect map $X \twoheadrightarrow Y$, then $X$ is compact.
\end{prop}

\begin{proof}
\pf
\step{1}{\pflet{$p : X \twoheadrightarrow Y$ be a perfect map.}}
\step{2}{For all $y \in Y$ and every open set $U$ such that $\inv{p}(y) \subseteq U$, there exists a neighbourhood $W$ of $y$ such that $\inv{p}(W) \subseteq U$.}
\begin{proof}
	\step{a}{\pflet{$y \in Y$}}
	\step{b}{\pflet{$U$ be an open set such that $\inv{p}(y) \subseteq U$.}}
	\step{c}{\pflet{$W = Y - p(X - U)$}}
	\step{d}{$W$ is open.}
	\begin{proof}
		\step{i}{$X - U$ is closed.}
		\begin{proof}
			\pf\ Proposition \ref{prop:open_closed}, \stepref{b}.
		\end{proof}
		\step{ii}{$p(X - U)$ is closed.}
		\begin{proof}
			\pf\ Since $p$ is a closed map (\stepref{1}).
		\end{proof}
		\step{iii}{$Y - p(X-U)$ is open.}
	\end{proof}
	\step{e}{$y \in W$}
	\begin{proof}
		\step{i}{\assume{for a contradiction $y \in p(X - U)$}}
		\step{ii}{\pick\ $x \in X - U$ such that $p(x) = y$}
		\step{iii}{$x \in \inv{p}(y)$}
		\step{iv}{$x \in U$}
		\begin{proof}
			\pf\ \stepref{b}
		\end{proof}
		...
		\end{proof}
	\end{proof}
	\step{f}{$\inv{p}(W) \subseteq U$}
	\begin{proof}
		\step{i}{\pflet{$x \in \inv{p}(W)$}}
		\step{ii}{$p(x) \in W$}
		\step{iii}{$p(x) \notin p(X - U)$}
		\begin{proof}
			\pf\ \stepref{c}
	...
	\end{proof}
\end{proof}
\step{3}{\pflet{$\mathcal{U}$ be an open cover of $X$.}}
\step{4}{For every $y \in Y$, there exists a neighbourhood $W$ of $y$ such that $\inv{p}(W)$ can be covered by finitely many elements of $\mathcal{U}$.}
\begin{proof}
	\step{a}{\pflet{$y \in Y$}}
	\step{b}{\pick\ finitely many sets $U_1, \ldots, U_n \in \mathcal{U}$ that cover $\inv{p}(y)$}
	\begin{proof}
		\pf\ Since $\inv{p}(y)$ is compact.
	\end{proof}
	\step{c}{There exists a neighbourhood $W$ of $y$ such that $\inv{p}(W) \subseteq U_1 \cup \cdots \cup U_n$.}
	\begin{proof}
		\pf\ \stepref{2}
	\end{proof}
\end{proof}
\step{5}{\pick\ finitely many open sets $W_1$, \ldots, $W_n$ in $Y$ such that each $\inv{p}(W_i)$ can be covered by finitely many elements of $\mathcal{U}$.}
\begin{proof}
	\pf\ Since $Y$ is compact.
\end{proof}
\step{6}{For $i=1, \ldots, n$, \pick\ a finite subset $\mathcal{U}_i$ of $\mathcal{U}$ such that $\inv{p}(W_i) \subseteq \bigcup \mathcal{U}_i$.}
\step{7}{$\mathcal{U}_1 \cup \cdots \cup \mathcal{U}_n$ covers $X$.}
\qed
\end{proof}
...
\begin{df}[Distance to a Set]
Let $X$ be a metric space. Let $A \subseteq X$ be nonempty and $x \in X$. The \emph{distance from $x$ to $A$} is
...
\begin{df}[(Open) Ball]
Let $X$ be a metric space. Let $x \in X$ and $r > 0$. The \emph{(open) ball} with \emph{centre} $x$ and \emph{radius} $r$ is
...
\step{2}{\assume{w.l.o.g. $X$ is nonempty.}}
\step{3}{\pick\ $a \in X$}
\step{4}{$\{ B(a,n) : n \in \mathbb{Z}_+ \}$ is an open cover of $X$.}
\step{5}{\pick\ a finite subcover $\{ B(a,n_1), \ldots, B(a,n_k) \}$.}
\step{6}{\pflet{$N = \max(n_1, \ldots, n_k)$}}
\step{7}{$X = B(a,N)$}
\qed
\end{proof}
...
\begin{prop}
Let $X$ be a metric space. Let $A \subseteq X$ be nonempty. The function $d(-,A) : X \rightarrow \mathbb{R}$ is continuous.
\end{prop}
...
\step{3}{\pflet{$y \in X$ with $d(x,y) < \delta$.}}
\step{4}{$d(x,A) < d(y,A) + \epsilon$}
\begin{proof}
	\step{a}{$\forall a \in A. d(x,a) < d(y,a) + \delta$}
	\begin{proof}
		\pf
		\begin{align*}
			d(x,a) & \leq d(y,a) + d(x,y) & (\text{Triangle Inequality}) \\
			& < d(y,a) + \delta & (\text{\stepref{3}})
		\end{align*}
	\end{proof}
	\step{b}{$\forall a \in A. d(x,A) < d(y,a) + \delta$}
	\step{c}{$\forall a \in A. d(x,A) - \delta < d(y,a)$}
	\step{d}{$d(x,A) - \delta \leq d(y,A)$}
	\step{e}{$d(x,A) \leq d(y,A) + \delta$}
	\step{f}{$d(x,A) < d(y,A) + \epsilon$}
\end{proof}
\step{5}{$d(y,A) < d(x,A) + \epsilon$}
\begin{proof}
	\pf\ Similar.
\end{proof}
...
\step{1}{\pflet{$(x,y) \in \mathbb{R}^2$ and $\epsilon > 0$}}
\step{2}{\pflet{$\delta = \epsilon / 2$}}
\step{3}{\pflet{$(x',y') \in (x-\delta, x + \delta) \times (y - \delta, y + \delta)$}}
\step{4}{$|x-x'|,|y-y'| < \delta$}
...
\begin{thm}
Let $X$ 	be a compact space. Let $f_n : X \rightarrow \mathbb{R}$ be a monotone increasing sequence of continuous functions, and $f : X \rightarrow \mathbb{R}$ be continuous. If $f_n$ converges to $f$ pointwise, then $f_n$ converges to $f$ uniformly.
\end{thm}

\begin{proof}
\pf
\step{1}{\pflet{$\epsilon > 0$}}
\step{2}{For all $x \in X$, there exists a neighbourhood $U$ of $x$ and an integer $N$ such that, for all $y \in U$, we have $|f_N(y) - f(y)| < \epsilon$}
\begin{proof}
	\step{a}{\pflet{$x \in X$}}
	\step{b}{\pick\ $N$ such that $\forall n \geq N. |f_n(x) - f(x)| < \epsilon / 3$.}
	\step{c}{\pick\ neighbourhoods $U_1$, $U_2$ of $x$ such that $\forall y \in U_1. |f_N(y) - f_N(x)| < \epsilon / 3$ and $\forall y \in U_. |f(y) - f(x)| < \epsilon / 3$.}
	\step{d}{\pflet{$U = U_1 \cap U_2$}}
	\step{e}{\pflet{$y \in U$}}
	\step{f}{$|f_N(y) - f(y)| < \epsilon$}
	\begin{proof}
		\pf
		\begin{align*}
			|f_N(y) - f(y)| & \leq |f_N(y) - f_N(x)| + |f_N(x) - f(x)| + |f(x) - f(y)| & (\text{Triangle Inequality}) \\
			& < \epsilon / 3 + \epsilon / 3 + \epsilon / 3 & (\text{\stepref{b}, \stepref{c}}) \\
			& = \epsilon
		\end{align*}
	\end{proof}
\end{proof}
\step{3}{\pick\ open sets $U_1$, \ldots, $U_k$ that cover $X$ such that, for all $i$, there exists $N_i$ such that $\forall y \in U_i. |f_{N_i}(y) - f(y)| < \epsilon$}
\step{4}{\pflet{$N = \max(N_1, \ldots, N_k)$}}
\step{5}{For all $x \in X$ we have $|f_N(x) - f(x)| < \epsilon$}
\step{5}{For all $n \geq N$ and $x \in X$ we have $|f_n(x) - f(x)| < \epsilon$}
\qed
\end{proof}

\begin{ex}
We cannot remove the requirement that $(f_n)$ is monotone increasing.

For all $n \in \mathbb{Z}_+$, define $f_n : [0,1] \rightarrow \mathbb{R}$ by
\[ f_n(x) = \frac{1}{n^3[x-(1/n)]^2 + 1} \enspace . \]
Then $f_n(x) \rightarrow 0$ as $n \rightarrow \infty$ for all $x$, but the convergence is not uniform.
\end{ex}

\begin{ex}
We cannot remove the requirement that $X$ is compact.

For all $n \in \mathbb{Z}_+$, define $f_n : [0,1) \rightarrow \mathbb{R}$ by $f_n(x) = -x^n$. Then $(f_n)$ is monotone increasing and $f_n(x) \rightarrow 0$ as $n \rightarrow \infty$ for all $x$, but the convergence is not uniform.
\end{ex}
...
\begin{thm}
Let $n$ be a positive integer. Let $A$ be a subspace of $\mathbb{R}^n$. Then $A$ is compact if and only if it is closed and bounded.
\end{thm}
...
\begin{proof}
	\pf\ Theorem \ref{thm:compact_closed}.
\end{proof}
\step{2}{If $A$ is compact then $A$ is bounded.}
\begin{proof}
	\pf\ Proposition \ref{prop:compact_bounded}.
\end{proof}
\step{3}{If $A$ is closed and bounded then $A$ is compact.}
\begin{proof}
	\step{a}{\assume{$A$ is closed and bounded.}}
	\step{b}{\pick\ $M$ such that $\forall \vec{x},\vec{y} \in A. \rho(\vec{x},\vec{y}) < M$.}
	\step{c}{\assume{w.l.o.g. $A \neq \emptyset$}}
	\step{d}{\pick\ $\vec{a} \in A$}
	\step{e}{$A \subseteq \prod_{i=1}^n [a_i - M, a_i + M]$}
	\step{f}{$A$ is compact.}
	\begin{proof}
		\pf\ Theorem \ref{thm:closed_compact}.
	\end{proof}
...
\begin{cor}
For $n \geq 1$, the unit sphere $S^{n-1}$ and the unit ball $B^n$ are compact.
\end{cor}
...
On $\mathbb{R}^n$ we call the uniform metric the \emph{square metric} and denote it by $\rho$.
...
Let $X$ be a metric space. Let $\mathcal{A}$ be an open covering of $X$. A \emph{Lebesgue number} for $\mathcal{A}$ is a real number $\delta > 0$ such that every bounded set with diameter $< \delta$ is included in some member of $\mathcal{A}$.
\end{df}
...
\pf
\step{1}{\pflet{$X$ be a compact metric space.}}
\step{2}{\pflet{$\mathcal{A}$ be an open cover of $X$.}}
\step{2.5}{\assume{w.l.o.g. $X \notin \mathcal{A}$}}
\step{3}{\pick\ $A_1, \ldots, A_n \in \mathcal{A}$ that cover $X$.}
\step{4}{For $i = 1, \ldots, n$, \pflet{$C_i = X - A_i$.}}
\step{5}{\pflet{$f : X \rightarrow \mathbb{R}$ be the function
\[ f(x) = \frac{1}{n} \sum_{i=1}^n d(x, C_i) \enspace . \]}}
\begin{proof}
	\pf\ Each $C_i$ is nonempty by \stepref{2.5}.
\end{proof}
\step{6}{$\forall x \in X. f(x) > 0$}
\begin{proof}
	\step{a}{\pflet{$x \in X$}}
	\step{b}{\pick\ $i$ such that $x \in A_i$}
	\step{c}{\pick\ $\epsilon > 0$ such that $B(x, \epsilon) \subseteq A_i$}
	\step{d}{$d(x,C_i) \geq \epsilon$}
	\step{e}{$f(x) \geq \epsilon / n$}
\end{proof}
\step{7}{\pflet{$\delta$ be the minimum value of $f(X)$.} \prove{$\delta$ is a Lebesgue number for $\mathcal{A}$.}}
\begin{proof}
	\pf\ $f(X)$ has a least element by the Extreme Value Theorem.
\end{proof}
\step{8}{\pflet{$B \subseteq X$ have diameter $< \delta$}}
\step{9}{\pick\ $x_0 \in B$}
\step{10}{$B \subseteq B(x_0, \delta)$}
\step{11}{\pflet{$m$ be such that $d(x_0,C_m) = \max(d(x_0,C_1), \ldots, d(x_0,C_n))$}}
\step{12}{$d(x_0,C_m) \geq \delta$}
\begin{proof}
	\pf
	\begin{align*}
		\delta & \leq f(x_0) & (\text{\stepref{7}}) \\
		& = \frac{1}{n} \sum_{i=1}^n d(x_0,C_i) & (\text{\stepref{5}}) \\
		& \leq \frac{1}{n} \sum_{i=1}^n d(x_0,C_m) & (\text{\stepref{11}}) \\
		& = d(x_0,C_m)
	\end{align*}
\end{proof}
\step{13}{$B \subseteq A_m$}
\begin{proof}
	\pf\ \stepref{10}, \stepref{12}
\end{proof}
\qed
\end{proof}
...
Let $X$ and $Y$ be metric spaces. Let $f : X \rightarrow Y$. Then $f$ is \emph{uniformly continuous} iff, for all $\epsilon > 0$, there exists $\delta > 0$ such that, for all $x,y \in X$, if $d(x,y) < \delta$ then $d(f(x),f(y)) < \epsilon$.
\end{df}
...
\step{3}{\pflet{$f : X \rightarrow Y$ be continuous.}}
\step{4}{\pflet{$\epsilon > 0$}}
\step{5}{\pick\ a Lebesgue number $\delta$ for $\{ \inv{f}(B(y, \epsilon / 2)) : y \in Y \}$.}
\step{6}{\pflet{$x,x' \in X$}}
\step{7}{\assume{$d(x,x') < \delta$}}
\step{8}{\pick\ $y \in Y$ such that $\{x,x'\} \subseteq \inv{f}(B(y, \epsilon / 2))$}
\step{9}{$d(f(x),f(x')) < \epsilon$}
\begin{proof}
	\pf
	\begin{align*}
		d(f(x),f(x')) & \leq d(f(x),y) + d(y,f(x')) & (\text{Triangle Inequality}) \\
		& < \epsilon / 2 + \epsilon / 2 & (\text{\stepref{8}}) \\
		& = \epsilon
	\end{align*}
\end{proof}
\qed
...
\begin{ex}
$GL(n,\mathbb{C})$ is a topological group.
\end{ex}
...
\begin{prop}
\label{prop:left_mult_homeomorphism}
...
\begin{prop}
\label{prop:right_mult_homeomorphism}
...
\begin{prop}
Let $G$ be a topological group. Let $A$ and $B$ be subspaces of $G$. If $A$ is closed and $B$ is compact, then $AB$ is closed.
\end{prop}

\begin{proof}
\pf
\step{1}{For all $c \in G - AB$, there exists a neighbourhood $W$ of $c$ such that $W \inv{B} \cap A = \emptyset$.}
\begin{proof}
	\step{a}{\pflet{$c \in G - AB$}}
	\step{b}{\pflet{$\phi : G^2 \rightarrow G$ be the function $\phi(x,y) = x \inv{y}$}}
	\step{c}{$\{c\} \times B \subseteq \inv{\phi}(G - A)$}
	\step{d}{\pick\ a neighbourhood $W$ of $c$ such that $W \times B \subseteq \inv{\phi}(G - A)$}
	\begin{proof}
		\pf\ Tube Lemma.
	\end{proof}
	\step{e}{$W \inv{B} \cap A = \emptyset$}
\end{proof}
\step{2}{For all $c \in G - AB$, there exists a neighbourhood $W$ of $c$ such that $W \subseteq G - AB$.}
\qed
\end{proof}

\begin{cor}
Let $G$ be a topological group. Let $H$ be a compact subgroup of $G$. Let $p : G \twoheadrightarrow G / H$ be  the quotient map. Then $p$ is a perfect map.
\end{cor}

\begin{proof}
\pf\ The only thing remaining to prove is that, for all $gH \in G/H$, we have $\inv{p}(gH)$ is compact. This holds because $\inv{p}(gH) = gH$ is homeomorphic to $H$. \qed
\end{proof}

\begin{cor}
Let $G$ be a topological group. Let $H$ be a compact subgroup of $G$. If $G/H$ is compact then $G$ is compact.
\end{cor}
