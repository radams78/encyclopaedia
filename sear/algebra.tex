...
\step{2}{For $\beta$ a discrete eigenvalue of $\beta$, let $c_\beta = \langle \phi_{f(\beta)} | \psi \rangle$}
\step{3}{For $\beta$ a continuous eigenvalue, let
\[ c'_\beta = g(\beta) \langle \phi_{f(\beta)} | \psi \rangle \]}
\step{4}{$\bra{\psi} = \sum_{\beta} \overline{c_\beta} \bra{\phi_{f(\beta)}} + \int_{\beta} c'_\beta g(\beta) \bra{\phi_{f(\beta)}}d \beta$}
\begin{proof}
	\pf
	\begin{align*}
