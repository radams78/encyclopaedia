...
\section{Injections and Surjections} % CHECKED
...
\begin{proof}
\pf
\step{1}{\assume{$f : B \twoheadrightarrow A$ is surjective.}}
\step{2}{\pick\ a function $g : A \rightarrow B$ such that, for all $x \in A$, we have $f(g(x)) = x$.}
\step{3}{\pflet{$x,y \in A$}}
\step{4}{\assume{$g(x) = g(y)$}}
\step{5}{$x = y$}
\begin{proof}
	\pf\ $x = f(g(x)) = f(g(y)) = y$
\end{proof}
...
\end{proof}
\step{2}{\pflet{$S = \{ x \in A : \bot \}$ with injection $i : S \rightarrow A$}}
\begin{proof}
