...
\begin{df}[Function]
Let $A$ and $B$ be sets and $F : A \looparrowright B$. Then $F$ is a \emph{function} from $A$ to $B$, $F : A \rightarrow B$, if and only if, for all $x : \El{A}$, there exists a unique $y : \El{B}$ such that $xFy$.
