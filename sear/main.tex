...
\begin{itemize}
\item a set $\mathrm{Ob}(\mathcal{C})$ of \emph{objects}
\item for any objects $X$ and $Y$, a set $\mathrm{Mor}(X,Y)$ of \emph{morphisms} from $X$ to $Y$. We write $f : X \rightarrow Y$ for $f : \El{\mathrm{Mor}(X,Y)}$.
\item for any objects $X$, $Y$ and $Z$, a function $\circ : \mathrm{Mor}(Y,Z) \times \mathrm{Mor}(X,Y) \rightarrow \mathrm{Mor}(X,Z)$, called \emph{composition}.
\end{itemize}
such that:
\begin{itemize}
...
\begin{df}
Let $\mathbf{Set}$ be the category of small sets and functions.
\end{df}
...
Define the identity functor, constant functors.
...
\begin{df}
For any field $K$, let $\mathbf{Vect}_K$ be the category of small vector spaces over $K$ and linear transformations.
\end{df}
...
\begin{df}[Subcomplex]
Let $(X, \mathcal{E})$ be a CW-complex and $\mathcal{E}' \subseteq \mathcal{E}$. Let $X' = \bigcup \mathcal{E}'$. Then $(X', \mathcal{E}')$ is a \emph{subcomplex} of $(X, \mathcal{E})$ iff $(X', \mathcal{E}')$ is a CW-complex, $X'$ is closed in $X$, and for all $e \in \mathcal{E}'$ we have $\overline{e} \subseteq X'$.
\end{df}
