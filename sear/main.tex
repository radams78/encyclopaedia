...
\section{Surjective Functions} % CHECKED
...
\begin{df}[Subspace]
Let $X$ be a topological space and $(Y,i)$ a subset of $X$. The \emph{subspace topology} on $Y$ is $\mathcal{T} = \{ \inv{i}(U) : U \text{ is open in } X \}$.
...
\begin{proof}
	\pf\ Since $\bigcup \mathcal{U} = \inv{i}(\bigcup \{ V : \inv{i}(V) \in \mathcal{U}\})$.
\end{proof}
\step{2}{For all $U,V \in \mathcal{T}$ we have $U \cap V \in \mathcal{T}$}
\begin{proof}
	\pf\ Since $\inv{i}(U) \cap \inv{i}(V) = \inv{i}(U 
\cap V)$.
\end{proof}
